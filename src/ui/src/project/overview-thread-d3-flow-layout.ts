...
export function buildOverviewThreadD3FlowLayout(
  ...
  for (
    ...
    const junction = demand.side === "left" ? demand.group.inHub : demand.group.outHub;
  ...
  for (
    ...
    if (sourceLaneIndex === targetLaneIndex) {
      ...
      const sourceSide = physicalDirection === "left-to-right" ? "right" : "left";
      const targetSide = physicalDirection === "left-to-right" ? "left" : "right";
      const sourceHub = sourceSide === "right" ? sourceGroup.outHub : sourceGroup.inHub;
      const targetHub = targetSide === "left" ? targetGroup.inHub : targetGroup.outHub;
  ...
  const interLanePairs = [...interLaneEdgesByPair.entries()]
    .map(([pairKey, pairEdges]) => buildInterLanePair(pairKey, pairEdges, laneIndex))
...
function buildNodeFanInObstacles(
  ...
  return groups.filter((group) => group.key !== owningGroupKey).map((group) => ({
    key: group.key,
...
function addSameLaneRoute(
  ...
  const pairKey = directedPairKey(sourceGroup, targetGroup);
  const sides = sameLaneCableSides(sourceGroup, targetGroup);
  const sourceHub = sides.source === "left" ? sourceGroup.inHub : sourceGroup.outHub;
  const targetHub = sides.target === "left" ? targetGroup.inHub : targetGroup.outHub;
  ...
  if (sourceGroup.key !== targetGroup.key) {
    cableRoute = tryBuildCableFieldRoute(
      sourceHub,
      targetHub,
      sameLaneObstacles.filter((obstacle) => !endpointGroupKeys.has(obstacle.key)),
      {
