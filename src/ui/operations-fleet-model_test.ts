...
Deno.test("declared servers join observed systems by exact identity first", () => {
  ...
  assertEquals(view.cards.map((card) => card.id), ["build123d-sandbox"]);
...
Deno.test("a declared server without observed evidence is idle, never a card", () => {
  const fleet: CockpitFleetProjection = {
    servers: [
      {
        id: "syson",
  ...
  assertEquals(view.cards.map((card) => card.id), ["syson"]);
