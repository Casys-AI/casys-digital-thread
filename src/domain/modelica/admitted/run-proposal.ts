...
export const SIMULATE_RUN_ADMITTED_MODELICA_OPERATION = Object.freeze(
  {
    id: "simulate.run-admitted-modelica",
    version: "1",
  } as const,
);
...
export const MODELICA_ADMITTED_RESULT_OUTPUT = Object.freeze(
  {
    role: "result",
    basename: "result.csv",
    mediaType: "text/csv",
...
export const MODELICA_ADMITTED_OUTPUT_MANIFEST = Object.freeze([
...
export type ModelicaAdmittedMinimumDestructionAssurance =
  | "acknowledged-unattested"
  | "proven";

export interface ModelicaAdmittedExecutionLimits {
...
export type ModelicaAdmittedLimitAssurance =
  | "backend-attested"
  | "broker-observed-cap"
  | "unattested";

export type ModelicaAdmittedLimitAssuranceMatrix = {
  readonly [Key in keyof ModelicaAdmittedExecutionLimits]:
    ModelicaAdmittedLimitAssurance;
};

export interface ModelicaAdmittedRunAdmission {
  readonly schemaVersion: typeof MODELICA_ADMITTED_RUN_ADMISSION_SCHEMA;
  readonly admissionArtifact: {
    readonly schemaVersion: typeof MODELICA_ADMITTED_COMPILED_ADMISSION_SCHEMA;
    readonly id: string;
    readonly fingerprint: ContentFingerprint;
  };
  readonly compilation: {
    readonly document: {
      readonly schemaVersion: typeof MODELICA_ADMITTED_COMPILATION_SCHEMA;
      readonly fingerprint: ContentFingerprint;
      readonly status: "ready-for-review";
    };
    readonly projection: {
      readonly target: "modelica-source-qualification";
      readonly fingerprint: ContentFingerprint;
      readonly status: "ready-for-review";
    };
    readonly source: {
    ...
    readonly profile: {
      readonly id: typeof MODELICA_ADMITTED_COMPILATION_PROFILE_ID;
      readonly version: string;
      readonly fingerprint: ContentFingerprint;
    };
  };
  readonly execution: {
    readonly profile: {
      readonly id: typeof MODELICA_ADMITTED_EXECUTION_PROFILE.id;
      readonly version: typeof MODELICA_ADMITTED_EXECUTION_PROFILE.version;
      readonly fingerprint: ContentFingerprint;
    };
    ...
    readonly runtime: {
      ...
      readonly limits: ModelicaAdmittedExecutionLimits;
      readonly limitAssurance: ModelicaAdmittedLimitAssuranceMatrix;
    };
    readonly outputValidator: ModelicaAdmittedOutputValidatorRef;
    readonly outputs: typeof MODELICA_ADMITTED_OUTPUT_MANIFEST;
    readonly minimumDestructionAssurance: ModelicaAdmittedMinimumDestructionAssurance;
  };
...
const PARAMETER_PREFIX = "simulate.modelica.admitted";
...
const LIMIT_KEYS = [
  ...
  "maxOutputFileBytes",
  "maxOutputTotalBytes",
] as const satisfies readonly (keyof ModelicaAdmittedExecutionLimits)[];

const FIXED_PARAMETER_COUNT = 60;

export function encodeModelicaAdmittedRunAdmissionParameters(
  value: unknown,
): readonly EngineeringDecisionProposalParameter[] {
  const admission = validateModelicaAdmittedRunAdmission(value);
  return deepFreeze(
...
export function parseModelicaAdmittedRunAdmissionParameters(
  parameters: readonly EngineeringDecisionProposalParameter[],
): ModelicaAdmittedRunAdmission {
  ...
  const parsed = validateModelicaAdmittedRunAdmission({
    ...
    compilation: {
      document: {
        schemaVersion: requireLiteral(
          ...
          MODELICA_ADMITTED_COMPILATION_SCHEMA,
        ),
        fingerprint: requireFingerprint(
          values,
    ...
    execution: {
      profile: {
        id: requireLiteral(
          ...
          MODELICA_ADMITTED_EXECUTION_PROFILE.id,
        ),
        version: requireLiteralVersion(
          values,
          `${PARAMETER_PREFIX}.profile.version`,
          MODELICA_ADMITTED_EXECUTION_PROFILE.version,
        ),
        fingerprint: requireFingerprint(
          values,
      ...
      runtime: {
        ...
        ])) as unknown as ModelicaAdmittedExecutionLimits,
        ...
        ])) as unknown as ModelicaAdmittedLimitAssuranceMatrix,
      },
      outputValidator: {
      ...
      outputs: [
        ...
        },
        {
          role: requireLiteral(
            ...
            MODELICA_ADMITTED_RESULT_OUTPUT.role,
          ),
          basename: requireLiteral(
            values,
            `${PARAMETER_PREFIX}.output.result.basename`,
            MODELICA_ADMITTED_RESULT_OUTPUT.basename,
          ),
          mediaType: requireLiteral(
            values,
            `${PARAMETER_PREFIX}.output.result.mediaType`,
            MODELICA_ADMITTED_RESULT_OUTPUT.mediaType,
          ),
          format: requireLiteral(
            values,
            `${PARAMETER_PREFIX}.output.result.format`,
            MODELICA_ADMITTED_RESULT_OUTPUT.format,
          ),
        },
      ],
...
export function validateModelicaAdmittedRunAdmission(
  value: unknown,
  path = "$modelicaAdmittedRunAdmission",
): ModelicaAdmittedRunAdmission {
  ...
  if (
    ...
    execution.outputs.length !== MODELICA_ADMITTED_OUTPUT_MANIFEST.length
  ) {
    throw new TypeError(
      `${path}.execution.outputs must declare evidence and result.`,
    );
  }
  for (const [index, expected] of MODELICA_ADMITTED_OUTPUT_MANIFEST.entries()) {
  ...
  return deepFreeze({
    ...
    compilation: {
      document: {
        schemaVersion: MODELICA_ADMITTED_COMPILATION_SCHEMA,
...
function parameterSpecs(
  ...
  add(
    `${PARAMETER_PREFIX}.schemaVersion`,
    "Admitted Modelica run admission schema",
  ...
  add(
    ...
    "Admitted Modelica source ID",
    admission.compilation.source.id,
  );
  add(
    `${PARAMETER_PREFIX}.compilation.source.sourceSha256`,
    "Admitted Modelica source SHA-256",
    admission.compilation.source.sourceFingerprint.digest,
  );
  add(
    `${PARAMETER_PREFIX}.compilation.source.captureSha256`,
    "Admitted Modelica source capture SHA-256",
    admission.compilation.source.captureFingerprint.digest,
  );
  add(
    `${PARAMETER_PREFIX}.compilation.source.analysisSha256`,
    "Admitted Modelica source analysis SHA-256",
  ...
  if (specs.length !== FIXED_PARAMETER_COUNT) {
    throw new TypeError(
      "Admitted Modelica execution MRTR grammar is internally inconsistent.",
    );
  }
  return specs;
}
...
function parseLimits(
  value: unknown,
  path: string,
): ModelicaAdmittedExecutionLimits {
...
function parseLimitAssuranceMatrix(
  value: unknown,
  path: string,
): ModelicaAdmittedLimitAssuranceMatrix {
  const record = exactRecord(value, LIMIT_KEYS, path);
...
function minimumDestructionAssurance(
  value: unknown,
  path: string,
): ModelicaAdmittedMinimumDestructionAssurance {
