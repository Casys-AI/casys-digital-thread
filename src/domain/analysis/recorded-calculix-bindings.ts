...
import type { FeaProofCaseCapture } from "./fea-proof-case-capture.ts";
...
export const RECORDED_CALCULIX_PROOF_MEDIA_TYPE = "application/json" as const;
export const RECORDED_CALCULIX_GEOMETRY_KIND = "step" as const;
export const RECORDED_CALCULIX_GEOMETRY_MEDIA_TYPE = "model/step" as const;

export type RecordedCalculixBindingDiagnosticCode =
  | "basis-latest"
  | "basis-mismatch"
  | "basis-absent"
  | "basis-ambiguous"
...
export interface RecordedCalculixBindingDiagnostic {
  readonly code: RecordedCalculixBindingDiagnosticCode;
  readonly artifactId: string | null;
  readonly artifactKind: string | null;
  readonly message: string;
}

export interface RecordedCalculixResolvedBindings {
 ...
 */
export function recordedCalculixReviewProposal(
...
export function recordedCalculixBindingRejectionMessage(input: {
...
export function diagnoseRecordedCalculixProofArtifact(
  artifact: Pick<ThreadArtifact, "id" | "kind" | "mediaType">,
): RecordedCalculixBindingDiagnostic | undefined {
  if (
    artifact.kind === RECORDED_CALCULIX_PROOF_KIND &&
    artifact.mediaType === RECORDED_CALCULIX_PROOF_MEDIA_TYPE
  ) {
    return undefined;
  }
  return {
    ...
    message: `proofCase binding "${artifact.id}" is kind=${artifact.kind} ` +
      `mediaType=${artifact.mediaType ?? "absent"}; recorded CalculiX requires ` +
      `a JSON document (kind: document, mediaType: application/json).`,
  };
}

export function diagnoseRecordedCalculixGeometryArtifact(
  artifact: Pick<ThreadArtifact, "id" | "kind" | "mediaType">,
): RecordedCalculixBindingDiagnostic | undefined {
  if (
    artifact.kind === RECORDED_CALCULIX_GEOMETRY_KIND &&
    artifact.mediaType === RECORDED_CALCULIX_GEOMETRY_MEDIA_TYPE
  ) {
    return undefined;
  }
  if (artifact.kind === "cad-model") {
    return {
      ...
      message: `geometry binding "${artifact.id}" is a cad-model capture. ` +
        "Recorded CalculiX @2 binds the canonical part STEP (kind: step), never the assembly cad-model or the @3 isolated authority.",
    };
  }
  return {
 ...
 * binding.
 */
export function resolveRecordedCalculixRunBindings(
  snapshot: ThreadSnapshot,
  proofArtifact: ThreadArtifact,
  capture: FeaProofCaseCapture,
):
  | { readonly status: "resolved"; readonly resolved: RecordedCalculixResolvedBindings }
  | {
    readonly status: "unresolved";
    readonly diagnostics: readonly RecordedCalculixBindingDiagnostic[];
  } {
  const diagnostics: RecordedCalculixBindingDiagnostic[] = [];
  const proofDiag = diagnoseRecordedCalculixProofArtifact(proofArtifact);
  ...
  if (!step) {
    ...
    const geometryDiag = diagnoseRecordedCalculixGeometryArtifact(step);
  ...
  return {
    ...
    resolved: {
      operation: {
        id: RECORDED_CALCULIX_RUN_OPERATION.id,
        version: RECORDED_CALCULIX_RUN_OPERATION.version,
        bindings,
      },
      proofArtifact,
      stepArtifact: step,
...
export function rejectCadModelGeometryLookalikes(
...
): readonly RecordedCalculixBindingDiagnostic[] {
  ...
  return [{
    ...
    message:
      `@2 geometry must be the canonical part STEP, never a cad-model or the @3 isolated authority. ` +
