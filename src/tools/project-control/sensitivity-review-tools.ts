...
import type { ProjectSensitivityStudySealReviewUseCase } from "../../application/ports/in/project-sensitivity-study-seal-review.ts";
import {
...
const CASE_ID_SCHEMA = {
  ...
  description:
    "Server-owned sensitivity-study catalog id. Unknown ids yield unresolved; the caller never supplies a path, JSON, or cadSource.",
} as const;
...
const projectSensitivityStudySealReviewTool: MCPTool = {
  ...
  description:
    "Compile analyze.seal-sensitivity-study@1 MRTR parameters and catalog-owned work/decision identities. Name the project; caseId and basis are optional (unique catalog template, unique current Thread tip — not latest). cadSource is the unique readable compile.seal-admission@1 admission whose source binds the template target.semanticKey. Not design.write-geometry@1, a cad-model, a STEP, or design.seal-isolated-geometry@1. Only an appendable review against the exact current project head is resolved and carries next.append / next.propose. A historical basis, catalog-absent project (including desk-lamp-dl06), or unbound semanticKey is unavailable or unresolved — never resolved. The caller never supplies mesh, loads, boxes, hashes or cadSource. Read-only: no project, Thread, MRTR or solver authority.",
  inputSchema: {
    type: "object",
    properties: {
