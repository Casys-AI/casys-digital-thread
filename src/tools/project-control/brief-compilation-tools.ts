...
function registerRequirements(
  ...
  app.registerTool(projectBriefRequirementsReviewTool, async (args) => {
    ...
    const content = result.status === "resolved"
      ? "Brief requirements review is resolved against the exact human-approved canonical brief. The returned model.write-requirements@2 parameters carry the signed approved-brief identity, exact brief-content fingerprint, and source item plus declared threshold for every scalar requirement. Source references are server-derived from the reopened brief; the server did not read the item prose, so the signing human still confirms each value against its statement. Construct a later model.write-requirements@2 proposal only from the returned decisionParameters; if none are present, do not invent them. This wrote no EngineeringProject or Thread state and granted no MRTR authority."
