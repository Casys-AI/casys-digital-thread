...
const projectAdmittedSpiceRunReviewTool: MCPTool = {
  ...
  description:
    "Prepare the exact human-review identity and canonical MRTR parameters for one future admitted SPICE closed-subset operating-point execution. The caller names only projectId; the server reopens the unique current Thread tip, selects its unique fresh non-archived canonical digital-thread compile.seal-admission@1 document whose compilation target/source is spice-circuit-source, and joins it to the server-owned execution profile. Concurrent CAD or Modelica admissions are not candidates. Missing, stale, archived, malformed, foreign-producer, CAD-only, Modelica-only, or ambiguous SPICE admissions fail closed. This provider-free read performs no code execution, returns no source bytes or runtime capability, mutates no EngineeringProject or Thread state, and grants no MRTR, provider, or dispatch authority. SPICE text, Thread and artifact identities, runtime, isolation, output, profile, command, tool, image, path, observations and transport facts remain server-owned. This is not mcp-spice and not project_led_driver_source_capture.",
  inputSchema: {
    type: "object",
    properties: {
      projectId: TECHNICAL_ID_SCHEMA,
