/** Durable monotone WAL for admitted SPICE isolated execution. */

import {
  type AdmittedSpiceExecutionAttempt,
  type AdmittedSpiceExecutionAttemptIdentity,
  type AdmittedSpiceExecutionAttemptKey,
  type AdmittedSpiceExecutionAttemptStore,
  type AdmittedSpiceExecutionDispatch,
  type AdmittedSpiceExecutionDispatchTransition,
  type AdmittedSpiceExecutionGenerationRecovery,
  type AdmittedSpiceExecutionThreadArtifactEvidence,
  type AdmittedSpiceExecutionThreadEvidence,
  type AdmittedSpiceExecutionThreadEvidenceInput,
  type AdmittedSpiceProvenDestruction,
  fingerprintAdmittedSpiceExecutionAttemptIdentity,
...
import { validateAdmittedSpiceExecutionProfile } from "./execution-profile-catalog.ts";

export const ADMITTED_SPICE_EXECUTION_ATTEMPT_SCHEMA =
  "spice-admitted-execution-attempt/1.0" as const;

const NO_PROGRESS = "Admitted SPICE execution WAL made no durable write progress.";
const MAX_WAL_BYTES = 1_048_576;

export class AdmittedSpiceExecutionAttemptIntegrityError extends Error {
  constructor(message: string) {
    super(message);
    this.name = "AdmittedSpiceExecutionAttemptIntegrityError";
  }
}

export class FileAdmittedSpiceExecutionAttemptStore
  implements AdmittedSpiceExecutionAttemptStore {
  readonly #directory: string;

  constructor(
    directory = "state/local/recorded-analysis/electrical/spice/admitted/attempts",
  ) {
  ...
  async read(
    ...
    try {
      ...
      throw integrity("The admitted SPICE execution WAL is not JSON.");
    }
    const attempt = await validateAdmittedSpiceExecutionAttempt(
      parsed,
      projectId,
      agentRunId,
    );
    if (`${deterministicJson(attempt)}\n` !== text) {
      throw integrity("The admitted SPICE execution WAL is not canonical.");
    }
    return attempt;
  }

  async prepare(
    identityValue: AdmittedSpiceExecutionAttemptIdentity,
    ...
    if (preparedAt !== identity.startedAt) {
      throw integrity(
        "The admitted SPICE WAL prepared time differs from its exact run start.",
      );
    }
    const attemptFingerprint = await fingerprintAdmittedSpiceExecutionAttemptIdentity(
      identity,
    );
    const fresh: AdmittedSpiceExecutionAttempt = deepFreeze({
      schemaVersion: ADMITTED_SPICE_EXECUTION_ATTEMPT_SCHEMA,
    ...
    return await this.#withLock(identity.projectId, identity.agentRunId, async () => {
      ...
      if (current) {
        ...
        if (
          ...
          throw integrity(
            "The admitted SPICE execution identity conflicts with its WAL.",
          );
        }
        return current;
      }
  ...
  markDispatching(
    ...
    return this.#dispatchTransition(input, (current) => {
      ...
      if (dispatchedAt !== current.identity.startedAt) {
        throw integrity(
          "Admitted SPICE generation-zero dispatch time differs from its exact run start.",
        );
      }
      if (current.phase === "dispatching") {
        ...
        throw integrity("Admitted SPICE generation-zero dispatch is divergent.");
      }
      if (current.phase !== "prepared") {
        throw integrity("Admitted SPICE generation-zero dispatch is out of order.");
      }
      return deepFreeze({
        ...base(current),
        phase: "dispatching",
  ...
  markGenerationZeroCleaned(
    ...
    return this.#transition(input, (current) => {
      ...
      if (current.phase === "generation-zero-cleaned") {
        ...
        throw integrity(
          "Admitted SPICE generation-zero cleanup proof is divergent.",
        );
      }
      if (
        ...
        throw integrity("Admitted SPICE generation-zero cleanup is out of order.");
      }
      return deepFreeze({
        ...base(current),
        phase: "generation-zero-cleaned",
        dispatch: current.dispatch as AdmittedSpiceExecutionDispatch & {
  ...
  markRedispatching(
    ...
    return this.#dispatchTransition(input, async (current) => {
      ...
      if (dispatchedAt !== current.identity.startedAt) {
        throw integrity(
          "Admitted SPICE generation-one dispatch time differs from its exact run start.",
        );
      }
      ...
      if (current.phase === "dispatching") {
        ...
        throw integrity("Admitted SPICE generation-one dispatch is divergent.");
      }
      if (current.phase !== "generation-zero-cleaned") {
        throw integrity("Admitted SPICE generation-one dispatch is out of order.");
      }
      const generationRecovery: AdmittedSpiceExecutionGenerationRecovery = deepFreeze({
  ...
  markOutputPublished(
    ...
    return this.#transition(input, async (current) => {
      if (
        ...
        throw integrity("Admitted SPICE output publication is out of order.");
      }
      ...
      if (current.phase !== "dispatching") {
        ...
        throw integrity("Admitted SPICE output publication is divergent.");
      }
      return deepFreeze({
        ...base(current),
  ...
  markCompleted(
    ...
    return this.#transition(input, async (current) => {
      if (current.phase === "completed") {
        ...
        throw integrity("Admitted SPICE Thread completion proof is divergent.");
      }
      if (current.phase !== "output-published") {
        throw integrity(
          "Admitted SPICE execution cannot complete before output publication.",
        );
      }
      const threadEvidence = await sealThreadEvidence(
        input.threadEvidence,
  ...
  markExecutionRejected(
    input: AdmittedSpiceExecutionAttemptKey & {
      readonly diagnostic: Parameters<
        typeof validateIsolatedCodeExecutionRejectionDiagnostic
      >[0];
    ...
    return this.#transition(input, (current) => {
      ...
      if (current.phase === "execution-rejected") {
        if (
          deterministicJson(current.rejection) ===
            deterministicJson({ diagnostic, destruction })
        ) return current;
        throw integrity("Admitted SPICE execution rejection is divergent.");
      }
      if (current.phase !== "dispatching") {
        throw integrity("Admitted SPICE execution rejection is out of order.");
      }
      return deepFreeze({
        ...base(current),
        ...
        dispatch: current.dispatch,
        generationRecovery: current.generationRecovery,
        rejection: { diagnostic, destruction },
      });
    });
  }
  ...
  markRetryGenerationClosed(
    ...
    return this.#transition(input, (current) => {
      ...
      const closedGeneration = deepFreeze({
        producerGeneration: 1 as const,
        destruction,
      });
      if (current.phase === "retry-generation-closed") {
        if (
          deterministicJson(current.closedGeneration) ===
            deterministicJson(closedGeneration)
        ) return current;
        throw integrity("Admitted SPICE retry generation closure is divergent.");
      }
      if (
        current.phase !== "dispatching" ||
        current.dispatch.dispatchCount !== 2 ||
        ...
        throw integrity(
          "Admitted SPICE retry generation closure is out of order.",
        );
      }
      return deepFreeze({
        ...base(current),
        phase: "retry-generation-closed" as const,
        dispatch: current.dispatch as AdmittedSpiceExecutionDispatch & {
  ...
  async pathFor(projectIdValue: string, agentRunIdValue: string): Promise<string> {
    ...
    const key = await sha256Fingerprint({
      schemaVersion: "spice-admitted-execution-wal-key/1.0",
      projectId,
      agentRunId,
    });
  ...
  async #transition(
    keyValue: AdmittedSpiceExecutionAttemptKey,
    transition: (
      current: AdmittedSpiceExecutionAttempt,
    ) =>
    ...
    return await this.#withLock(key.projectId, key.agentRunId, async () => {
      ...
      if (!current) {
        throw integrity("The admitted SPICE execution WAL is missing.");
      }
  ...
  async #dispatchTransition(
    ...
    transition: (
      ...
      | Extract<AdmittedSpiceExecutionAttempt, { phase: "dispatching" }>
      | Promise<
        Extract<AdmittedSpiceExecutionAttempt, { phase: "dispatching" }>
      >,
  ): Promise<AdmittedSpiceExecutionDispatchTransition> {
  ...
  async #writeNew(attempt: AdmittedSpiceExecutionAttempt): Promise<void> {
  ...
  async #replace(attempt: AdmittedSpiceExecutionAttempt): Promise<void> {
    await this.#ensureDirectory();
    const path = await this.pathFor(attempt.projectId, attempt.agentRunId);
    await writeWalFileDurably(
      path,
  ...
  async #assertReread(expected: AdmittedSpiceExecutionAttempt): Promise<void> {
    ...
    if (!actual || deterministicJson(actual) !== deterministicJson(expected)) {
      throw integrity(
        "The admitted SPICE execution WAL failed its durable reread.",
      );
    }
  }
...
export async function validateAdmittedSpiceExecutionAttempt(
  value: unknown,
  ...
  literalValue(
    root.schemaVersion,
    ADMITTED_SPICE_EXECUTION_ATTEMPT_SCHEMA,
  ...
  if (
    ...
    !fingerprintsEqual(
      attemptFingerprint,
      await fingerprintAdmittedSpiceExecutionAttemptIdentity(identity),
    )
  ) {
    throw integrity("The admitted SPICE execution WAL identity is divergent.");
  }
  const baseValue = {
    schemaVersion: ADMITTED_SPICE_EXECUTION_ATTEMPT_SCHEMA,
    ...key,
  ...
  if (dispatch.dispatchedAt !== identity.startedAt) {
    throw integrity(
      "The admitted SPICE dispatch time differs from its exact run start.",
    );
  }
  if (phase === "generation-zero-cleaned") {
    ...
    return deepFreeze({
      ...baseValue,
      phase,
      dispatch: dispatch as AdmittedSpiceExecutionDispatch & {
        readonly dispatchCount: 1;
        readonly producerGeneration: 0;
      },
  ...
  if (phase === "retry-generation-closed") {
    if (
      dispatch.dispatchCount !== 2 || dispatch.producerGeneration !== 1 ||
      generationRecovery === null
    ) {
      throw integrity("Retry generation closure names another dispatch.");
    }
    const closedGeneration = exactRecord(
      root.closedGeneration,
      ["producerGeneration", "destruction"],
      "$attempt.closedGeneration",
    );
    literalValue(
      closedGeneration.producerGeneration,
      1,
...
async function validateIdentity(
  ...
  if (
    executionRunId !==
      await deriveAdmittedSpiceExecutionRunId(projectId, agentRunId)
  ) {
    throw integrity("The admitted SPICE execution run id is not server-derived.");
  }
  ...
  if (!fingerprintsEqual(decision.inputFingerprint, approval.inputFingerprint)) {
    throw integrity(
      "The admitted SPICE decision and approval fingerprints diverge.",
    );
  }
  ...
  const executionProfile = await validateAdmittedSpiceExecutionProfile(
...
function validateIsolatedRequest(
  ...
  admission: ReturnType<typeof validateSpiceAdmittedRunAdmission>,
  profile: Awaited<ReturnType<typeof validateAdmittedSpiceExecutionProfile>>,
): AdmittedSpiceExecutionAttemptIdentity["isolatedRequest"] {
  const root = exactRecord(value, [
    "schemaVersion",
    "runId",
    "producerGeneration",
...
function assertAdmissionProfileMatch(
  ...
  if (
    ...
    throw integrity(
      "The admitted SPICE MRTR and server execution profile diverge.",
    );
  }
}
...
async function validatePublishedReceipt(
  ...
  if (
    ...
    throw integrity(
      "The published admitted SPICE receipt differs from its durable identity.",
    );
  }
  return receipt;
}

function validateDispatch(value: unknown): AdmittedSpiceExecutionDispatch {
  ...
  if (
    ...
    throw integrity(
      "Admitted SPICE dispatch count and producer generation diverge.",
    );
  }
  return deepFreeze({
  ...
  } as AdmittedSpiceExecutionDispatch);
}

async function validateGenerationRecovery(
  value: unknown,
  executionRunId: string,
  dispatch: AdmittedSpiceExecutionDispatch,
): Promise<AdmittedSpiceExecutionGenerationRecovery | null> {
...
async function validateGenerationAdvance(
  ...
  try {
    ...
    throw integrity(
      "The producer-generation advance differs from this admitted SPICE run.",
    );
  }
}
...
function validateThreadEvidenceInput(
...
): AdmittedSpiceExecutionThreadEvidenceInput {
  const root = exactRecord(value, [
    "snapshotId",
    "revision",
    "subjectId",
  ...
  const expectedSnapshotId =
    `${identity.basis.subjectId}:r${expectedRevision}:simulate-run-admitted-spice-${identity.agentRunId}`;
  ...
  if (
    ...
    !evidenceOutput ||
    !resultOutput ||
    evidence.id !== `spice-admitted-evidence-${evidenceOutput.sha256}` ||
    evidence.fingerprint.digest !== evidenceOutput.sha256 ||
    result.id !== `spice-admitted-result-${resultOutput.sha256}` ||
    result.fingerprint.digest !== resultOutput.sha256
  ) {
    throw integrity(
      "The admitted SPICE Thread evidence is not the exact documentary successor.",
    );
  }
  return deepFreeze({
    snapshotId,
...
async function sealThreadEvidence(
  ...
  identity: AdmittedSpiceExecutionAttemptIdentity,
  receipt: IsolatedCodeExecutionReceiptRecord,
): Promise<AdmittedSpiceExecutionThreadEvidence> {
...
async function validateStoredThreadEvidence(
  ...
  if (
    ...
    throw integrity(
      "The admitted SPICE Thread evidence fingerprint is divergent.",
    );
  }
...
async function fingerprintThreadEvidence(
  identity: AdmittedSpiceExecutionAttemptIdentity,
  receipt: IsolatedCodeExecutionReceiptRecord,
  evidence: AdmittedSpiceExecutionThreadEvidenceInput,
) {
  return await sha256Fingerprint({
    schemaVersion: "spice-admitted-execution-thread-evidence/1.0",
    attemptIdentityFingerprint: await fingerprintAdmittedSpiceExecutionAttemptIdentity(
      identity,
    ),
...
function validateThreadArtifactEvidence(
...
): AdmittedSpiceExecutionThreadArtifactEvidence {
  ...
  if (id !== `spice-admitted-${role}-${fingerprint.digest}`) {
    throw integrity(`${path}.id does not derive from its exact fingerprint.`);
  }
  return deepFreeze({ id, fingerprint });
}

function phaseOf(value: unknown): AdmittedSpiceExecutionAttempt["phase"] {
  const root = exactRecord(value, [
    "schemaVersion",
    "projectId",
    "agentRunId",
...
function keysForPhase(
  phase: AdmittedSpiceExecutionAttempt["phase"],
): string[] {
  const keys = [
    "schemaVersion",
    "projectId",
...
function keyFor(
  identity: AdmittedSpiceExecutionAttemptIdentity,
  attemptFingerprint: AdmittedSpiceExecutionAttemptKey["attemptFingerprint"],
...
function validateKey(value: unknown): AdmittedSpiceExecutionAttemptKey {
  const root = exactRecord(value, [
    "projectId",
    "agentRunId",
    "executionRunId",
...
function assertKey(
  ...
  if (
    ...
    throw integrity("The admitted SPICE execution WAL key is divergent.");
  }
}

function base(attempt: AdmittedSpiceExecutionAttempt) {
  return {
...
function validateStorageRoot(value: unknown): string {
  if (
    ...
    throw new TypeError(
      "Admitted SPICE WAL directory must be a bounded path.",
    );
  }
...
function integrity(
  ...
  return new AdmittedSpiceExecutionAttemptIntegrityError(message);
}
