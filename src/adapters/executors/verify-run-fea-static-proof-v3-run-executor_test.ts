...
} from "../../domain/analysis/calculix-isolated-execution.ts";
import { CALCULIX_ISOLATED_OUTPUT_MANIFEST } from "../../domain/analysis/calculix-isolated-execution.ts";
import type { IsolatedCodeExecutionReceiptRecord } from "../../domain/analysis/isolated-code-execution.ts";
...
import { FileCalculixIsolatedProductAttemptStore } from "../wal/file-calculix-isolated-product-attempt-store.ts";
import {
  createRecordedCalculixV2Fixture,
  createRecordedCalculixV3Fixture,
...
Deno.test("isolated CalculiX @3 refuses the legacy @2 ROP before local solve or SysON", async () => {
  ...
  try {
    ...
    await assertRejects(
      () => executor.execute(RECORDED_CALCULIX_V2_FIXTURE_AGENT, fixture.command),
      TypeError,
...
interface Runtime {
  ...
  readonly fixture: Awaited<ReturnType<typeof createRecordedCalculixV3Fixture>>;
...
async function createRuntime(directory: string): Promise<Runtime> {
  const profiles = profileCatalog("a");
  const profile = await profiles.initial();
  const fixture = await createRecordedCalculixV3Fixture(directory, profile);
...
function executorForFixture(
  directory: string,
  fixture: Awaited<ReturnType<typeof createRecordedCalculixV2Fixture>>,
  options: {
