...
const basis = {
  ...
  runId: "run:printability-wal",
  caseDigest: "a".repeat(64),
  dispatchedAt: "2026-08-11T00:00:00.000Z",
};

Deno.test("printability WAL rereads and rejects a canonical capture whose fingerprint was tampered", async () => {
  const directory = await Deno.makeTempDir({ prefix: "casys-printability-wal-" });
  try {
    const store = new FileCm01DripTrayPrintabilityAttemptStore(directory);
    ...
    await assertRejects(() => store.begin(basis), PrintabilityRunOutcomeUnknownError);
  } finally {
    await Deno.remove(directory, { recursive: true });
  }
});

Deno.test("printability WAL writeAll handles partial writes exactly", async () => {
  const chunks: number[] = [];
  const file = {
