 ...
 * claims, invalidates a work item, proposes a rerun, or calls CalculiX.
 */
...
export interface MechanicalPreservationThreadSnapshotStore extends ThreadSnapshotStore {
  getFresh(snapshotId: string): Promise<ThreadSnapshot | undefined>;
}

export interface AnalyzeEvaluateMechanicalPreservationRunExecutorDependencies {
  readonly projects: Pick<EngineeringProjectRevisionStore, "get">;
  readonly commands: Pick<
    EngineeringProjectCommandService,
    "claimRun" | "publishRun" | "completeRun" | "failRun"
  >;
  readonly snapshots: MechanicalPreservationThreadSnapshotStore;
  readonly evaluation: EvaluateMechanicalPreservationUseCase;
  readonly captures: MechanicalPreservationCaptureStore;
  readonly lease: EngineeringProjectRunLease;
}

export class AnalyzeEvaluateMechanicalPreservationRunExecutor {
  readonly #projects: Pick<EngineeringProjectRevisionStore, "get">;
  readonly #commands:
    AnalyzeEvaluateMechanicalPreservationRunExecutorDependencies["commands"];
  readonly #snapshots: MechanicalPreservationThreadSnapshotStore;
  readonly #evaluation: EvaluateMechanicalPreservationUseCase;
  readonly #captures: MechanicalPreservationCaptureStore;
  readonly #lease: EngineeringProjectRunLease;
  ...
  constructor(
    dependencies: AnalyzeEvaluateMechanicalPreservationRunExecutorDependencies,
  ) {
  ...
  async execute(
    ...
    if (origin.kind !== "agent") {
      throw new EngineeringProjectCommandError(
        "permission_denied",
        "Only an authenticated agent can execute the provider-free mechanical preservation control.",
      );
    }
    ...
    try {
      ...
      if (run.status === "queued") {
        await this.#commands.claimRun(origin, {
          ...
          summary: "Started the provider-free mechanical preservation control.",
        });
        claimed = true;
      ...
      if (evaluated.status !== "resolved") {
        throw invalidTransition(
          `The provider-free mechanical preservation control is ${evaluated.status}: ` +
      ...
      if (
        ...
        throw new Error(
          "Mechanical preservation capture changed during exact readback.",
        );
      }
      ...
      const expectedSuccessor = buildSuccessor({
        ...
        decisionArtifactId: evaluated.decisionArtifactId,
      });
      ...
      if (
        ...
        throw new Error(
          "Mechanical-preservation ThreadSnapshot was not exactly readable after save.",
        );
      }
      ...
      if (run.status === "running") {
        await this.#commands.publishRun(origin, {
          ...
          summary: "Publishing the provider-free mechanical preservation capture.",
        });
      ...
      if (snapshotWriteMayHaveBeenDispatched) {
        ...
        if (completed) return completed;
        throw invalidTransition(
          "Mechanical-preservation Thread write may have been dispatched, but project attachment did not finish. Retry this exact command to reopen the deterministic successor.",
        );
      }
  ...
  async #assertCompletedEvidence(
    ...
    if (run.status !== "completed" || !run.resultSnapshot) {
      throw invalidTransition(
        "The mechanical preservation control did not complete through this exact run.",
      );
    }
    ...
    if (
      ...
      throw invalidTransition(
        "The completed mechanical preservation control lacks its exact direct Thread successor.",
      );
    }
    ...
    if (!artifact || artifact.kind !== "document") {
      throw invalidTransition(
        "The completed mechanical preservation evidence is not a Thread document.",
      );
    }
    ...
    if (
      ...
      throw invalidTransition(
        "The completed mechanical-preservation capture no longer equals its exact run.",
      );
    }
    const observedFingerprint = await sha256Fingerprint(capture);
    if (!fingerprintsEqual(observedFingerprint, artifact.fingerprint)) {
      throw invalidTransition(
        "The completed mechanical-preservation capture fingerprint no longer matches its Thread artifact.",
      );
    }
    if (
      artifact.uri !==
        crossDomainImpactMechanicalPreservationCaptureUri(observedFingerprint.digest)
    ) {
      throw invalidTransition(
        "The completed mechanical-preservation document does not retain its canonical capture URI.",
      );
    }
    const basisSnapshot = await exactBasisSnapshot(this.#snapshots, basis);
    const expected = buildSuccessor({
      ...
      decisionArtifactId: capture.decision.artifact.id,
    });
    if (deterministicJson(expected.snapshot) !== deterministicJson(validated)) {
      throw invalidTransition(
        "The completed mechanical-preservation successor is not the exact deterministic capture extension.",
      );
    }
  }
  ...
  async #recordFailure(
    ...
    try {
      ...
      await this.#commands.failRun(origin, {
        ...
        expectedRevision: project.revision,
        summary:
          "Mechanical preservation stopped before a ThreadSnapshot write was dispatched.",
        code: "analyze-evaluate-mechanical-preservation-not-published",
        message:
          "The provider-free mechanical preservation control stopped before its documentary capture was published.",
      });
    } catch {
...
function buildSuccessor(input: {
  ...
  if (
    ...
    input.capture.decision.artifact.id !== input.decisionArtifactId ||
    ...
    throw invalidTransition(
      "Mechanical-preservation capture does not match its exact trusted run and server-derived artifact inputs.",
    );
  }
  const inputArtifacts = exactInputArtifacts(
    input.basisSnapshot,
    input.artifactInputs,
  );
  const decision = inputArtifacts.filter((artifact) =>
    artifact.id === input.decisionArtifactId
  );
  if (decision.length !== 1 || decision[0]!.kind !== "document") {
    throw invalidTransition(
      "Mechanical-preservation capture does not name one exact impact-decision document on its basis.",
    );
  }
  const producer = {
    serverId: "digital-thread",
    tool:
      `${ANALYZE_EVALUATE_MECHANICAL_PRESERVATION_OPERATION.id}@${ANALYZE_EVALUATE_MECHANICAL_PRESERVATION_OPERATION.version}`,
    runId: input.run.id,
  } as const;
  const artifact: ThreadArtifact = {
    id:
      `cross-domain-impact-mechanical-preservation-${input.captureFingerprint.digest}`,
    name: "Mechanical preservation control",
    kind: "document",
  ...
  const consumptions: ThreadArtifactConsumption[] = inputArtifacts.map(
    (upstream) => ({
      id:
        `analyze-evaluate-mechanical-preservation-${input.run.id}:consume:${upstream.id}`,
  ...
  const extension: ThreadSnapshotExtension = {
    id: `analyze-evaluate-mechanical-preservation-${input.run.id}`,
    name: "Capture the provider-free mechanical preservation control",
    ...
    provenance: inputArtifacts.flatMap((upstream) => {
      const consumptionId =
        `analyze-evaluate-mechanical-preservation-${input.run.id}:consume:${upstream.id}`;
      const role = upstream.id === decision[0]!.id
        ? "impact-decision document"
        : "server-reread preservation input";
      return [
        {
          id:
            `analyze-evaluate-mechanical-preservation-${input.run.id}:derived-from:${upstream.id}`,
          relation: "derived_from" as const,
          from: { kind: "artifact" as const, id: artifact.id },
          to: { kind: "artifact" as const, id: upstream.id },
          rationale: `The mechanical-preservation capture reread this exact ${role}.`,
        },
        {
          id:
            `analyze-evaluate-mechanical-preservation-${input.run.id}:uses:${upstream.id}`,
          relation: "uses" as const,
          from: { kind: "consumption" as const, id: consumptionId },
          to: { kind: "artifact" as const, id: upstream.id },
          rationale:
            `The provider-free preservation control verified this exact ${role} fingerprint.`,
        },
      ];
    }),
    proposedActions: [],
  ...
  if (!applied.applied) {
    throw invalidTransition(
      "This exact mechanical preservation document is already present in the basis snapshot.",
    );
  }
...
function exactInputArtifacts(
  ...
  const artifacts = references.map((reference) => {
    if (artifactIds.has(reference.id)) {
      throw invalidTransition(
        "Mechanical-preservation capture repeats one Thread artifact input.",
      );
    }
    ...
    if (
      ...
      throw invalidTransition(
        "Mechanical-preservation capture names an artifact input absent or inexact on its Thread basis.",
      );
    }
    return matches[0]!;
  });
  if (artifacts.length === 0) {
    throw invalidTransition(
      "Mechanical-preservation capture has no exact Thread artifact inputs.",
    );
  }
  return artifacts;
}
...
function requireShape(
  ...
  if (
    ...
    !workItem ||
    operation?.id !== ANALYZE_EVALUATE_MECHANICAL_PRESERVATION_OPERATION.id ||
    operation.version !== ANALYZE_EVALUATE_MECHANICAL_PRESERVATION_OPERATION.version ||
    operation.bindings.length !== 1 ||
    operation.bindings[0]?.name !== "approvedBrief" ||
    operation.bindings[0].source.kind !== "approved-brief"
  ) {
    throw invalidTransition(
      `Run ${run.id} is not bound to ${ANALYZE_EVALUATE_MECHANICAL_PRESERVATION_OPERATION.id}@${ANALYZE_EVALUATE_MECHANICAL_PRESERVATION_OPERATION.version} with the sole approvedBrief binding.`,
    );
  }
}
...
function requireClaimedShape(
  ...
  if (
    ...
    throw invalidTransition(
      "This executor may continue only the exact mechanical-preservation run it claimed.",
    );
  }
}

async function exactBasisSnapshot(
  snapshots: MechanicalPreservationThreadSnapshotStore,
  ...
  if (
    ...
    throw invalidTransition(
      "The exact Thread basis snapshot is not available for mechanical preservation.",
    );
  }
...
function completionCommand(
  ...
  return {
    ...
    expectedRevision,
    summary: "Captured the provider-free mechanical preservation control.",
...
function exactCompletedEvidence(
  ...
  if (
    ...
    throw invalidTransition(
      "The completed mechanical-preservation run is not attached to exactly one declared document artifact.",
    );
  }
  ...
  if (
    ...
    throw invalidTransition(
      "The completed mechanical-preservation evidence reference is not the sealed document.",
    );
  }
  return evidence;
}
...
function commandStep(commandId: string, step: string): string {
  return `${commandId}:analyze-evaluate-mechanical-preservation:${step}`;
}
