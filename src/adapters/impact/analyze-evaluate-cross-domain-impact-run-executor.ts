/**
 * Provider-free executor for `analyze.evaluate-cross-domain-impact@1`.
 *
...
export interface CrossDomainImpactEvaluationThreadSnapshotStore extends ThreadSnapshotStore {
...
export class AnalyzeEvaluateCrossDomainImpactRunExecutor {
  readonly #projects: Pick<EngineeringProjectRevisionStore, "get">;
  readonly #commands: AnalyzeEvaluateCrossDomainImpactRunExecutorDependencies["commands"];
  readonly #snapshots: CrossDomainImpactEvaluationThreadSnapshotStore;
  readonly #evaluation: EvaluateCrossDomainImpactUseCase;
  readonly #captures: CrossDomainImpactEvaluationCaptureStore;
  readonly #lease: EngineeringProjectRunLease;
  ...
  async #executeLeased(
    ...
    try {
      ...
      if (!reopenedCapture || deterministicJson(reopenedCapture) !== deterministicJson(evaluated.capture)) {
        throw new Error("Cross-domain impact evaluation capture changed during exact readback.");
      }
      ...
      if (!snapshotReadback || deterministicJson(snapshotReadback) !== deterministicJson(expectedSuccessor.snapshot)) {
        throw new Error("Impact-evaluation ThreadSnapshot was not exactly readable after save.");
      }
      ...
      if (run.status === "running") {
        await this.#commands.publishRun(origin, {
          ...
          summary: "Publishing the provider-free cross-domain impact evaluation capture.",
        });
  ...
  async #requiredProject(projectId: string): Promise<EngineeringProjectSnapshot> {
    ...
    if (!project) {
      throw new EngineeringProjectCommandError("project_not_found", `Project ${projectId} was not found.`);
    }
    return project;
  }
  ...
  async #assertCompletedEvidence(
    ...
    if (run.status !== "completed" || !run.resultSnapshot) {
      throw invalidTransition("The cross-domain impact analysis did not complete through this exact run.");
    }
    const resultSnapshot = run.resultSnapshot;
    const snapshot = await this.#snapshots.getFresh(resultSnapshot.snapshotId);
    if (!snapshot || snapshot.id !== resultSnapshot.snapshotId ||
      snapshot.revision !== resultSnapshot.revision || snapshot.subject.id !== resultSnapshot.subjectId ||
      snapshot.revision !== basis.revision + 1 || snapshot.subject.id !== basis.subjectId ||
      snapshot.previous?.snapshotId !== basis.snapshotId || snapshot.previous.revision !== basis.revision
    ) {
      throw invalidTransition("The completed cross-domain impact analysis lacks its exact direct Thread successor.");
    }
    ...
    if (!artifact || artifact.kind !== "document") {
      throw invalidTransition("The completed cross-domain impact analysis evidence is not a Thread document.");
    }
    const capture = await this.#captures.read(artifact.fingerprint);
    if (!capture || capture.trustedRunId !== run.id || capture.evaluatedAt !== requiredStart(run)) {
      throw invalidTransition("The completed impact-evaluation capture no longer equals its exact run.");
    }
    const observedFingerprint = await sha256Fingerprint(capture);
    if (!fingerprintsEqual(observedFingerprint, artifact.fingerprint)) {
      throw invalidTransition("The completed impact-evaluation capture fingerprint no longer matches its Thread artifact.");
    }
    if (artifact.uri !== crossDomainImpactEvaluationCaptureUri(observedFingerprint.digest)) {
      throw invalidTransition("The completed impact-evaluation document does not retain its canonical capture URI.");
    }
    const basisSnapshot = await exactBasisSnapshot(this.#snapshots, basis);
    ...
    if (deterministicJson(expected.snapshot) !== deterministicJson(validated)) {
      throw invalidTransition("The completed impact-evaluation successor is not the exact deterministic capture extension.");
    }
  }
  ...
  async #recordFailure(
    ...
    try {
      ...
      await this.#commands.failRun(origin, {
        ...
        expectedRevision: project.revision,
        summary: "Cross-domain impact analysis stopped before a ThreadSnapshot write was dispatched.",
        code: "analyze-evaluate-cross-domain-impact-not-published",
        message: "The provider-free cross-domain impact analysis stopped before its documentary capture was published.",
      });
    } catch {
...
function buildSuccessor(input: {
  ...
  if (input.capture.evaluatedAt !== evaluatedAt || input.capture.trustedRunId !== input.run.id ||
    input.capture.manifestSeal.artifact.id !== input.manifestSealArtifactId ||
    deterministicJson(input.capture.artifactInputs) !== deterministicJson(input.artifactInputs) ||
    input.captureUri !== crossDomainImpactEvaluationCaptureUri(input.captureFingerprint.digest)
  ) {
    throw invalidTransition("Impact-evaluation capture does not match its exact trusted run and server-derived artifact inputs.");
  }
  const inputArtifacts = exactInputArtifacts(input.basisSnapshot, input.artifactInputs);
  const manifestSeal = inputArtifacts.filter((artifact) => artifact.id === input.manifestSealArtifactId);
  if (manifestSeal.length !== 1 || manifestSeal[0]!.kind !== "document") {
    throw invalidTransition("Impact-evaluation capture does not name one exact manifest-seal document on its basis.");
  }
  const producer = {
    serverId: "digital-thread",
    tool: `${ANALYZE_EVALUATE_CROSS_DOMAIN_IMPACT_OPERATION.id}@${ANALYZE_EVALUATE_CROSS_DOMAIN_IMPACT_OPERATION.version}`,
    runId: input.run.id,
  } as const;
  ...
  const extension: ThreadSnapshotExtension = {
    ...
    provenance: inputArtifacts.flatMap((upstream) => {
      const consumptionId = `analyze-evaluate-cross-domain-impact-${input.run.id}:consume:${upstream.id}`;
      const role = upstream.id === manifestSeal[0]!.id
        ? "sealed manifest document"
        : "server-reread impact input";
      return [
        {
          id: `analyze-evaluate-cross-domain-impact-${input.run.id}:derived-from:${upstream.id}`,
        ...
        {
          id: `analyze-evaluate-cross-domain-impact-${input.run.id}:uses:${upstream.id}`,
          relation: "uses" as const,
          from: { kind: "consumption" as const, id: consumptionId },
          to: { kind: "artifact" as const, id: upstream.id },
          rationale: `The provider-free evaluator verified this exact ${role} fingerprint.`,
        },
      ];
    }),
    proposedActions: [],
  ...
  if (!applied.applied) {
    throw invalidTransition("This exact cross-domain impact evaluation document is already present in the basis snapshot.");
  }
...
function exactInputArtifacts(
  ...
  const artifacts = references.map((reference) => {
    if (artifactIds.has(reference.id)) {
      throw invalidTransition("Impact-evaluation capture repeats one Thread artifact input.");
    }
    ...
    if (matches.length !== 1 || !fingerprintsEqual(matches[0]!.fingerprint, reference.fingerprint)) {
      throw invalidTransition("Impact-evaluation capture names an artifact input absent or inexact on its Thread basis.");
    }
    return matches[0]!;
  });
  if (artifacts.length === 0) {
    throw invalidTransition("Impact-evaluation capture has no exact Thread artifact inputs.");
  }
  return artifacts;
}
...
function requireClaimedShape(
  ...
  if (run.claimedBy?.origin !== origin.kind || run.claimedBy.id !== origin.actorId) {
    throw invalidTransition("This executor may continue only the exact impact-evaluation run it claimed.");
  }
}
...
async function exactBasisSnapshot(
  ...
  if (!snapshot || snapshot.id !== basis.snapshotId || snapshot.revision !== basis.revision ||
    snapshot.subject.id !== basis.subjectId
  ) {
    throw invalidTransition("The exact Thread basis snapshot is not available for cross-domain impact evaluation.");
  }
...
function exactCompletedEvidence(
  ...
  if (!workItem || declared.length !== 1 || run.evidenceRefs.length !== 1 ||
    workItem.evidenceRefs.length !== 1 || !sameEvidenceRefs(run.evidenceRefs, workItem.evidenceRefs)
  ) {
    throw invalidTransition("The completed impact-evaluation run is not attached to exactly one declared document artifact.");
  }
  const evidence = run.evidenceRefs[0]!;
  if (evidence.snapshotId !== snapshot.id || evidence.snapshotRevision !== snapshot.revision ||
    evidence.kind !== "artifact"
  ) {
    throw invalidTransition("The completed impact-evaluation evidence reference is not the sealed document.");
  }
  return evidence;
}
