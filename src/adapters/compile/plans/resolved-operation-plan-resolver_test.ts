...
Deno.test("ResolvedOperationPlanResolver resolves canonical Modelica evidence on a true successor lineage deterministically", async () => {
  ...
  const resolver = new ResolvedOperationPlanResolver(fixture.dependencies);
...
Deno.test("ResolvedOperationPlanResolver rejects the historical structured Uint8Array hash false-green", async () => {
...
Deno.test("ResolvedOperationPlanResolver rejects noncanonical normalized manifest bytes", async () => {
...
Deno.test("ResolvedOperationPlanResolver never interprets persisted provider snake-case wire JSON as the qualified document", async () => {
...
Deno.test("ResolvedOperationPlanResolver rejects a Thread artifact URI that aliases another CAS object", async () => {
...
Deno.test("ResolvedOperationPlanResolver rejects out-of-range Modelica values after canonical manifest validation", async () => {
...
Deno.test("ResolvedOperationPlanResolver rejects a simulation case from an unrelated Thread lineage", async () => {
...
Deno.test("ResolvedOperationPlanResolver rejects a run fingerprint transplanted across registered inputs", async () => {
  const fixture = await modelicaFixture();
...
Deno.test("ResolvedOperationPlanResolver rejects a qualification authority manifest CAS transplant", async () => {
...
Deno.test("ResolvedOperationPlanResolver rejects a transplanted MRTR approval", async () => {
  const fixture = await modelicaFixture();
  const approval =
...
Deno.test("ResolvedOperationPlanResolver rejects a completed Modelica seal without its result snapshot and evidence", async () => {
  const fixture = await modelicaFixture();
...
Deno.test("ResolvedOperationPlanResolver rejects a completed Modelica seal result snapshot transplanted from another lineage", async () => {
  const fixture = await modelicaFixture();
...
Deno.test("ResolvedOperationPlanResolver rejects a tampered completed Modelica seal result snapshot", async () => {
  const fixture = await modelicaFixture();
...
Deno.test("ResolvedOperationPlanResolver rejects a completed Modelica seal with a missing result evidence ref", async () => {
  const fixture = await modelicaFixture();
  const sealRun = recordedSealRun(fixture.input);
...
Deno.test("ResolvedOperationPlanResolver rejects a Modelica seal result ordered before its declared basis on the same lineage", async () => {
