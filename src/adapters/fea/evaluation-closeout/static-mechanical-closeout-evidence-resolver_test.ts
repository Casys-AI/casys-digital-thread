...
Deno.test(
  ...
  async () => {
    ...
    try {
      ...
      assertEquals(resolved.criteria.every((criterion) => criterion.status === "pass"), true);
      ...
      const step = wrongStep.artifacts.find((artifact) => artifact.kind === "step")! as {
        mediaType?: string;
      };
      ...
      const evaluation = stale.evaluations[0] as unknown as {
        freshness: { status: string; changedAt: string; invalidatedByChangeIds: string[] };
      };
      ...
      mutableAmbiguous.evaluations.push(structuredClone(mutableAmbiguous.evaluations[0]));
      await assertRejects(
        () => resolveStaticMechanicalCloseoutEvidence(fixture.dependencies, {
          project: fixture.project,
          basis: fixture.basis,
...
Deno.test(
  ...
  async () => {
    ...
    try {
      ...
      const divergentProject = structuredClone(passed.project) as EngineeringProjectSnapshot;
    ...
    const failed = await createCompletedStaticMechanicalCloseoutFixture({ status: "fail" });
    try {
      const resolved = await resolveStaticMechanicalCloseoutEvidence(failed.dependencies, {
