...
const identity = {
  ...
  step: "architecture-insert" as const,
  dispatchedAt: "2026-08-03T08:00:00.000Z",
};

Deno.test("r3 architecture write journal refuses an unknown provider outcome instead of replaying it", async () => {
  const directory = await Deno.makeTempDir({
    prefix: "casys-inspection-drone-architecture-attempt-",
  });
  try {
    const store = new FileInspectionDroneArchitectureAttemptStore(directory);
...
Deno.test("r3 architecture write journal resumes only an exact recorded normalized acknowledgement", async () => {
  const directory = await Deno.makeTempDir({
    prefix: "casys-inspection-drone-architecture-attempt-",
  });
  try {
    ...
    const completion = {
      ...
      result: {
        inserted: "true",
        parentId: "syson-root-package-1",
        textSha256: "a2c8dcfa65418ef3cc57d1c4ad8e10d66a488db683b530fffdc5a76ed465a2f9",
      },
    };
...
Deno.test("r3 architecture write journal treats an unreadable existing marker as an unknown outcome", async () => {
  const directory = await Deno.makeTempDir({
    prefix: "casys-inspection-drone-architecture-attempt-",
  });
  try {
...
Deno.test("r3 architecture write journal atomically replaces a dispatched record with its completed acknowledgement", async () => {
  ...
  await store.complete({
    ...identity,
    completedAt: "2026-08-03T08:00:01.000Z",
    result: { inserted: "true", parentId: "syson-root-package-1" },
  });
