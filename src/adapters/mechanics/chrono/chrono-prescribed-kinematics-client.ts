...
/**
 * Fixed mcp-chrono 0.3.5 adapter.  It exposes no provider, tool, endpoint, or
...
function parseReceipt(
  ...
  literal(packageIdentity.version, "0.3.5", `${path}.package.version`);
  ...
  literal(providerIdentity.version, "0.3.5", `${path}.provider.version`);
...
function providerErrorCode(value: unknown, path: string): string {
  ...
  if (!CHRONO_PROVIDER_ERROR_CODES.has(code)) {
    throw protocol(`${path} is not a published mcp-chrono 0.3.5 error code`);
  }
  return code;
}
