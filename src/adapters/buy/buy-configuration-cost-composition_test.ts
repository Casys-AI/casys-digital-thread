...
Deno.test(
  ...
  async () => {
    ...
    try {
      ...
      assertEquals(sealedArtifact?.inputArtifactIds, [candidateArtifact!.id]);
      assertEquals(
        sealedSnapshot?.provenance.some((link) =>
          link.relation === "derived_from" && link.from.id === sealedArtifact!.id &&
          link.to.id === candidateArtifact!.id
        ),
        true,
      );
