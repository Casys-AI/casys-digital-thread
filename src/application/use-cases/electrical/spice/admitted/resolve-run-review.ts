...
export type ResolveProjectAdmittedSpiceRunReviewErrorCode =
  | "invalid_request"
  | "project_not_found"
...
export class ResolveProjectAdmittedSpiceRunReviewError extends Error {
  constructor(
    readonly code: ResolveProjectAdmittedSpiceRunReviewErrorCode,
    message: string,
  ) {
    super(message);
    this.name = "ResolveProjectAdmittedSpiceRunReviewError";
  }
}

export interface AdmittedSpiceRunReviewProjectReader {
  get(projectId: string): Promise<EngineeringProjectSnapshot | undefined>;
}

export interface AdmittedSpiceRunReviewSnapshotReader {
...
export interface ResolveProjectAdmittedSpiceRunReviewDependencies {
  readonly projects: AdmittedSpiceRunReviewProjectReader;
  readonly snapshots: AdmittedSpiceRunReviewSnapshotReader;
  ...
  readonly exactReview: ProjectAdmittedSpiceRunReviewUseCase;
}

export class ResolveProjectAdmittedSpiceRunReview
  implements ProjectAdmittedSpiceRunReviewUseCase {
  constructor(
    private readonly dependencies: ResolveProjectAdmittedSpiceRunReviewDependencies,
  ) {}
  ...
  async execute(value: unknown): Promise<ProjectAdmittedSpiceRunReviewResult> {
    ...
    const artifact = await this.#selectUniqueFreshSpiceAdmission(
      snapshot,
      request.projectId,
      basis,
    );
    const command = deepFreeze<ProjectAdmittedSpiceRunReviewCommand>({
  ...
  async #selectUniqueFreshSpiceAdmission(
    ...
    for (const artifact of canonical) {
      if (
        await this.#isSpiceCompilationAdmission(artifact, projectId, basis)
      ) {
    ...
    if (candidates.length === 0) {
      throw resolutionError(
        "admission_not_found",
        "The current Thread tip has no fresh digital-thread compile.seal-admission@1 SPICE compilation.",
      );
    }
    if (candidates.length !== 1) {
      throw resolutionError(
        "admission_ambiguous",
        `The current Thread tip has ${candidates.length} fresh digital-thread compile.seal-admission@1 SPICE compilations; the server will not choose one.`,
      );
    }
    return candidates[0]!;
  }

  async #isSpiceCompilationAdmission(
...
function parseRequest(value: unknown): ProjectAdmittedSpiceRunReviewRequest {
  try {
...
function resolutionError(
  code: ResolveProjectAdmittedSpiceRunReviewErrorCode,
  message: string,
): ResolveProjectAdmittedSpiceRunReviewError {
  return new ResolveProjectAdmittedSpiceRunReviewError(code, message);
}
