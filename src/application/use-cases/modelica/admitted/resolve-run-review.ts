...
export class ResolveProjectAdmittedModelicaRunReview
  ...
  async #selectUniqueFreshModelicaAdmission(
    ...
    if (candidates.length === 0) {
      throw resolutionError(
        "admission_not_found",
        "The current Thread tip has no fresh digital-thread compile.seal-admission@2 Modelica compilation.",
      );
    }
    if (candidates.length !== 1) {
      throw resolutionError(
        "admission_ambiguous",
        `The current Thread tip has ${candidates.length} fresh digital-thread compile.seal-admission@2 Modelica compilations; the server will not choose one.`,
      );
    }
    return candidates[0]!;
  }
