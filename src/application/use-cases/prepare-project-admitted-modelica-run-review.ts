/**
 * Provider-free preparation of an admitted Modelica execution review.
 *
 * Reopens one sealed compilation admission, joins it to the server-owned
 * Modelica closed-subset execution profile, and derives MRTR parameters.
...
} from "../ports/in/project-admitted-modelica-run-review.ts";
import {
...
} from "../ports/out/admitted-modelica-execution-profile-catalog.ts";
import type {
...
export class ProjectAdmittedModelicaRunReviewError extends Error {
  constructor(
    readonly code: ProjectAdmittedModelicaRunReviewErrorCode,
    message: string,
  ) {
    super(message);
    this.name = "ProjectAdmittedModelicaRunReviewError";
  }
}

export interface PrepareProjectAdmittedModelicaRunReviewDependencies {
  readonly admissions: TechnicalCompilationAdmissionReader;
  readonly profiles: AdmittedModelicaExecutionProfileCatalog;
}

export class PrepareProjectAdmittedModelicaRunReview
  implements ProjectAdmittedModelicaRunReviewUseCase {
  readonly #admissions: TechnicalCompilationAdmissionReader;
  readonly #profiles: AdmittedModelicaExecutionProfileCatalog;

  constructor(dependencies: PrepareProjectAdmittedModelicaRunReviewDependencies) {
  ...
  async execute(value: unknown): Promise<ProjectAdmittedModelicaRunReviewResult> {
    let command: ProjectAdmittedModelicaRunReviewCommand;
    try {
    ...
    let compilation: ReadyModelicaCompilation;
    try {
      compilation = await reopenReadyModelicaCompilation(reopened, command);
    } catch {
      throw reviewError(
        "admission_integrity_failed",
        "The reopened technical-compilation admission is not an exact, singular, ready Modelica compilation.",
      );
    }

    let rawProfile: AdmittedModelicaExecutionProfile;
    try {
      rawProfile = await this.#profiles.initial();
    } catch {
      throw reviewError(
        "execution_profile_unavailable",
        "The server-owned admitted Modelica execution profile is unavailable.",
      );
    }
    ...
    try {
      ...
      const decisionParameters = encodeModelicaAdmittedRunAdmissionParameters(
        admission,
      );
      const reparsed = parseModelicaAdmittedRunAdmissionParameters(
        decisionParameters,
      );
      const reencoded = encodeModelicaAdmittedRunAdmissionParameters(reparsed);
      if (deterministicJson(reencoded) !== deterministicJson(decisionParameters)) {
        throw new TypeError("Admitted Modelica MRTR replay is not canonical.");
      }
      return deepFreeze({ admission: reparsed, decisionParameters: reencoded });
    } catch {
      throw reviewError(
        "execution_profile_integrity_failed",
        "The server-owned Modelica execution profile does not exactly match the sealed compilation.",
      );
    }
  }
}

interface ReadyModelicaCompilation {
...
function parseCommand(value: unknown): ProjectAdmittedModelicaRunReviewCommand {
  ...
  const basis = parseExactThreadSnapshotBasis(
    command.basis,
    "$admittedModelicaRunReview.basis",
  );
  const artifactFingerprint = validateContentFingerprint(
    command.artifactFingerprint,
    "$admittedModelicaRunReview.artifactFingerprint",
  );
  const artifactId = safeId(
    command.artifactId,
    "$admittedModelicaRunReview.artifactId",
  );
  if (
    artifactId !==
...
async function reopenReadyModelicaCompilation(
  value: unknown,
  command: ProjectAdmittedModelicaRunReviewCommand,
): Promise<ReadyModelicaCompilation> {
  ...
  if (
    projection.target !== "modelica-source-qualification" ||
    projection.profile.target !== "modelica-source-qualification" ||
    projection.profile.id !== MODELICA_ADMITTED_COMPILATION_PROFILE_ID ||
    ...
    throw new TypeError("The sole projection is not a ready Modelica projection.");
  }
  ...
  if (
    ...
    throw new TypeError("The sole Modelica source or profile identity disagrees.");
  }
  ...
  if (!fingerprintsEqual(sourceFingerprint, admittedSource.sourceFingerprint)) {
    throw new TypeError("The Modelica source bytes disagree with their identity.");
  }
...
function assertExecutionProfileMatchesCompilation(
  ...
  compilation: ReadyModelicaCompilation,
): void {
...
function deriveExecutionAdmission(
  ...
  const admission: ModelicaAdmittedRunAdmission = {
  ...
  const parameters = encodeModelicaAdmittedRunAdmissionParameters(admission);
  return parseModelicaAdmittedRunAdmissionParameters(parameters);
}

function reviewError(
  code: ProjectAdmittedModelicaRunReviewErrorCode,
  message: string,
): ProjectAdmittedModelicaRunReviewError {
  return new ProjectAdmittedModelicaRunReviewError(code, message);
}
