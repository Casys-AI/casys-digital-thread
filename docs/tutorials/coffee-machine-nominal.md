...
Build and start the Console server. The Console MCP App used in this tutorial remains a
read-only observer; the server's separate project-control tools are outside this flow:
...
verdict. Use the Workbench tab or the
[native Workbench preview](../how-to/preview-native-workbench.md) for the linked product
surface.
