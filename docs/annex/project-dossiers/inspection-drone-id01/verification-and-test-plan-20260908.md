...
| G0.2 | What is physically installed?                 | current architecture, COTS leads and missing-item census                                                                  | freeze include/exclude rows for every occurrence, including one GNSS choice, telemetry, a distinct RC link, cables, fasteners, guards and retention                         | human for inclusion; supplier or measurement for facts | an envelope occurrence or packaged ground radio is not an installed vehicle item     |
...
| G0.5 | Which battery and power topology are exact?   | battery screens; exclusive PM02/PDB, PM02/four-in-one and PM06 branches; UBEC lead; no viable current+keep-in combination | select one pack and one complete main-path topology; draw every net, measurement plane, branch, regulator, connector and protection element                                 | human                                                  | catalogue C-rate and supply ratings are not delivered mission current or consumption |
...
| G0.8 | What image geometry makes the mission useful? | provisional exact camera variant; target/feature and inspection claim still unresolved                                    | apply the [camera-to-target geometry basis](camera-mission-geometry-basis-20260908.md) only after stand-off, capture mode, incidence and image criterion are human-approved | human/domain inspection criterion owner                | nominal object-plane sampling is not defect detectability or installed image quality |
...
| E1 | after mission and configuration | Is usable energy sufficient for the named phases?                                        | phase durations/powers, measured auxiliary duty, exact pack curve or discharge record, current, cutoff, temperature and age | reproducible algebraic phase sum; when needed, guarded discharge with synchronized U/I/temperature and integrated `∫UI dt`             | human reserve/cutoff policy               | nameplate Wh, C-rate and `Wh_nameplate / P_bench` are not usable energy      |
...
| C4  | after prototype             | Are vibration, magnetic, radio and conducted/radiated coupling controlled?                 | installed rotors, camera/CSI mode, GNSS, harness, antennas, current paths and sensor logs | restrained measurements, GNSS/log and spectrum analysis, plus external EMC work where required           | human/laboratory criteria                 | the documented Camera Module 3 GPS-L1 RFI risk is not closed by spacing assumptions |
...
If a later physical test is authorized and captured, its measurements remain evidence of
that exact article, setup and protocol. They are not automatically stored as a
sensitivity result. `verify.evaluate-sensitivity-base@1` remains a separate explicit
registered evaluation over its admitted sensitivity basis; no implicit conversion or
backfill is proposed here.
...
campaign occurred. A later four-task Grok-native pass returned `HOLD` on usable-energy,
installed-avionics, power-chain and mass closure. Codex independently checked the
official GNB, Holybro and Raspberry Pi facts, retained the smallest new entrance
evidence above and rejected an extra hypothetical mass worksheet. No Astra or Terra
escalation was needed for that pass. A subsequent three-task Grok-native pass reviewed
official Camera Module 3 facts, first-order image geometry and dossier placement. Codex
reopened the primary Raspberry Pi sources, recalculated the retained normalized lookup
and added G0.8 without selecting a criterion, stand-off, capture mode or physical test.
