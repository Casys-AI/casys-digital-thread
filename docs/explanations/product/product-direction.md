...
| **Make**   | Can we fabricate this STEP?         | Measured DFM (`industrialize.run-dfm-checks@1`); recorded DFM App over that capture. Printability / print-estimate stay documentary         | Named violation only. Does not authorize a `z*` or a geometry write. A recorded `pass` is not restored MRTR authority                           |
| **Buy**    | What is the configuration and cost? | `buy.capture-configuration-cost@1` then `buy.seal-configuration-cost@1` over a sourced configuration, canonical STEP and dated ERP captures | A missing qualified ERP binding stays unresolved. Partial coverage stays partial. Behave/Make verdicts do not transfer. Recorded App ≠ live BOM |
...
`source.dependency-lowering-unavailable`. A dead assignment or a hash-sealed photo of
constructor literals is not a dimensioned drawing and cannot feed the sensitivity grid.
This is the behave CAD compiler invariant for every new admission, not a
project-specific rule. Embedded profile-1 compilation documents retain their historical
replay semantics; already-sealed STEP photos, including Heron `design.write-geometry@1`,
are not rewritten. New canonical STEP comes only from `project_admitted_geometry_export`
after a parameterized admission. `design.write-geometry@1` refuses a preview photo draft
(`admission_required`). Modelica qualification is out of this rule.
