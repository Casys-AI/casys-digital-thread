...
| --------- | ---------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | -------------------------------------------------------------------------------------------------------- | ----------------------------------------------------------------------------------------------------------- |
| Build123d | 473 public names of the pinned 0.11.1 + the finite Python host core                                                                                                          | `config/build123d-api/inventory-0.11.1.json`, regenerated by introspecting the sandbox image             | analyzer **1.7.0** / profile **3.1.0** qualified slice. Inventory is documentary until F1 generates tables. |
...
Analyzer **1.7.0** / profile **3.1.0** also accept physical standalone-module comments
as non-semantic provenance annotations. Those comments do not prove geometry, admission,
a parameter binding, or execution; qualified syntax and admitted source remain the
evidence. The hand table still includes `Ellipsoid`, which the 0.11.1 inventory does not
list. The next proposed family is **F1** (generated tables + analyzer 2.0.0), not an
inferred coverage claim.
