...
| Conditional evaluation (L4) | `verify.evaluate-admitted-modelica-observations@1` is registered and conditional: SysON compares exact admitted observations to a reviewed thermal method sheet. A unit-identity mismatch stays `unresolved`. An L4 `pass` is not a product verdict and is not lamp proof. |
