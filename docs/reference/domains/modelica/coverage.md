...
- Treating raw CAS as Modelica source ingress. A `.mo` file enters through
  `project_resource_capture` then `project_technical_source_capture` (`resourceRef`). A
  thermal method sheet may be interpreted at upload through the existing typed store.
  Admitted execution still starts from `compile.seal-admission@2`.
