...
`compile.seal-admission@1` under `modelica-closed-subset-v2` / `2.0.0`. Callers supply
...
calls `omc` directly with DASSL. It is not an MCP Modelica-provider call. Network,
image pull, worker identity, executable allow-list, paths, and timeout are server-owned.
...
CSV columns to the declared outputs in source order. It rejects non-finite values,
wrong columns, invalid starts, or incomplete experiment-grid coverage.
