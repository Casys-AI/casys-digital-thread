...
> Verified-Against: 99c4498f (2026-08-29).
...
Authority:
[`sensitivity-study-v3.ts`](../../../../src/domain/sensitivity/study/sensitivity-study-v3.ts).
...
| Method supports / loads          | Non-empty; unique ids; provider-neutral                                | Enforced non-empty; **unbounded** upper count             | Same                                    |
...
The method cannot carry a provider, tool or response-schema field. The server-owned
binding chooses those only after a separate runtime qualification.
