...
| Live SysON graph (package, PartDefinitions, PartUsages, AttributeUsages) | Duplicate PartDef labels fail closed; ratchet indexes by Maps/Sets; no size check before index construction | Enforced identity/uniqueness failures; **unbounded** nodes and edges | Same. H03 may add a bound only after a reviewed decision. |
