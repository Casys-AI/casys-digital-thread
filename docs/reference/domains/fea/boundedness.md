...
### Proof-artifact anti-removal history

`assertProofCaseArtifactNotRemoved` in
[`verify-seal-proof-case-run-executor.ts`](../../../../src/adapters/fea/seal-case/verify-seal-proof-case-run-executor.ts)
walks the immutable predecessor chain to the root when the exact proof digest is absent
from the current snapshot. A currently present matching proof is a fast path. The
predicate is the exact `(subjectId, proofDigest)` pair. A previously carried matching
artifact is a removal refusal. An unresolved, mismatched, wrong-subject, or cyclic
ancestor is `proof_case_lineage_review_required`. The check occurs before the execution
claim, after `assertThreadSnapshotLineageIntact`. Ancestor cardinality is **unbounded**;
the walk-to-root is **enforced**. There is no local ancestor-count ceiling and no caller
flag or local approval that short-circuits the walk. A successful source/seal review
does not prove that this execution-time history check will pass.
...
(`MAXIMUM_MANIFEST_BYTES = 1_048_576`; nine-role `CALCULIX_ISOLATED_OUTPUT_MANIFEST`),
...
Nine output roles, in code order: `input.step`, `request.json`, `mesh.geo`, `mesh.inp`,
`gmsh.log`, `job.inp`, `ccx.log`, `job.dat`, `result.json`.
