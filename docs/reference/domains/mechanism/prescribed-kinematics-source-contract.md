...
ProjectSourceWorkspace. It declares a scenario; it does not select Chrono, a runtime,
an MCP tool, an engineering method, or a verdict.
...
| `assembly.partUsageElementId` | Exact assembly `PartUsage`, distinct from every body usage |
...
ids use `[A-Za-z][A-Za-z0-9_-]{0,63}`, the tree must already be directed from the ground,
parent and child frames must match exactly, both axes must be literal `[0,0,1]`, and the
schedule may have at most 510 intervals. The lowerer never composes a missing transform.
