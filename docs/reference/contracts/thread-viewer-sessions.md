...
CalculiX `io.casys.mcp-calculix.results@0.8.6` is registered only from an exact
fresh `verify.run-fea-static-proof@3` isolated result, its matching
`input.step`, and the reopened `calculix-isolated-execution-evidence`. The
registrar uses the same
`state/local/recorded-analysis/calculix/isolated-execution/evidence` store as
server composition; it preserves the original run, result, evidence and
canonical provider session fingerprint. Modelica
`io.casys.mcp-modelica.results@0.6.4` is registered only from a fresh
`simulate.run-admitted-modelica@1` result plus its exact canonical
`modelica-admitted-execution-capture/2.0`, admission, evidence and result
artifact membership. Its basis subject must literally be `project:<projectId>`.

After a one-time compatible package installation, these two bindings are
backfilled by ordinary reconciliation and refreshed automatically on a Project
revision; no solver, provider, Project or Thread write occurs. To add or update
a viewer, publish its whole App first, install one exact App version and
resource in `packages.json`, then add a narrow factory that reopens the
provider's recorded evidence and schema rather than teaching the Workbench the
provider payload. Test both the provider parser and reconciliation before
enabling it.

Chrono has no registrar binding here. Its published App `0.3.4` parser requires
`provenance.server.version = 0.3.4`, while the registered execution provider is
pinned to `0.3.2`; package installation alone cannot make that evidence
compatible. This needs a separate provider-App compatibility change, not a
forged session or version mapping.
