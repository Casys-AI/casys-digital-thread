...
Next AST lock: Polygon; `shell`; `Plane()` / `Axis()` constructors; `Location` /
`Vector` / class `Scale`; extrude `both=`/`dir=`/`until=`; offset `openings=` / `kind=`;
revolve `revolution_arc=`; named `Axis` bindings; fillet / chamfer method forms; general
`MemberExpression`, `.faces()`, `filter_by`; math `sin`/`cos` and every other D4 math
name; `&` / `|` until D4 admits the tokens; loft / sweep / `Align` / `mirror`.
