...
Qualified Build123d 1.5.0 extends the 1.4.0 subset with `Ellipse`/`RegularPolygon`
sketches, positional extrude amount, short `fillet`/`chamfer`,
`fillet(edges(), positional radius)`, and math scalars `pi`/`e`/`tau`. Previously
qualified bundles stay bit-identical; the public analysis identity does not change for
existing sources. A sketch is never a valid `result`. Same-kind `&` is parsed in the
frontend but D4 rejects the token before analysis.

Next AST lock: Polygon; Plane placements; named `Pos`/`Rot` bindings; extrude
`taper=`/`both=`/`dir=`/`until=`; fillet / chamfer method forms; general
`MemberExpression`, `.faces()`, `filter_by`; math `sin`/`cos` and every other D4 math
name; `&` / `|` until D4 admits the tokens.
