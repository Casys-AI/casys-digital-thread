...
| `build123d-closed-subset-v1` (`build123d-qualified-lezer` **1.4.0**) | Python / build123d     | `Box`, `Cylinder`, `Cone`, `Sphere`, `Torus`, `Ellipsoid`, `Wedge`, `Rectangle`, `Circle`, `Pos`, `Rot`, `Compound`; `+`/`-` same-kind; left-associative `Pos`/`Rot` chains and `Pos *`/`Rot *` solid or sketch; `scale(solid, scalar)`; `fillet(solid.edges(), radius=scalar)`; `chamfer(solid.edges(), length)`; `extrude(sketch, amount=scalar)`; numeric params; one solid `result` | D4-allowed but unproven syntax; a sketch is never a valid result; do not open general MemberExpression, `.faces()`, or `filter_by` |
...
Qualified Build123d 1.4.0 extends the 1.3.0 subset with left-associative `Pos`/`Rot`
placement chains and `Rot *` sketch. Previously qualified bundles stay bit-identical;
the public analysis identity does not change for existing sources. A sketch is never a
valid `result`.

Next AST lock: Ellipse, Polygon, RegularPolygon and Plane placements; named `Pos`/`Rot`
bindings; extrude `taper=`/`both=`/`dir=`/`until=` and a positional amount;
`fillet(solid, r)` and `chamfer(solid, l)` positional-solid forms.
