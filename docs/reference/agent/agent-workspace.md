...
| `project_technical_compilation_preview`                   | Evidence CAS write | `{ projectId, sourceRefs: [capture.result.reference] }`. Every locator must resolve to one shared ProjectSourceWorkspace basis. The normal response is a server-owned summary of at most 8 KiB plus immutable `evidenceRef`; read diagnostics, gaps, source-manifest, source-text, projections, ready decision-parameters, the exact operation, or explicit full-evidence through `project_technical_compilation_preview_detail`. Full evidence is mandatory for MRTR; reuse the detail operation verbatim in `project_change_append`. Build123d 3.1 lowers only the direct scalar-leaf closure; Modelica and circuit-only SPICE multi-file closures stay `unresolved` / `source.dependency-lowering-unavailable`. |
...
`binding.missing`. The normal preview result is a bounded summary and immutable
`evidenceRef`; it does not expose the compilation document. Call the detail tool with
only `projectId`, that reference, one named section and an optional opaque cursor.
`full-evidence` is explicit and mandatory before MRTR. This is a breaking
`structuredContent` migration: replace prior direct reads of `document`,
`decisionParameters`, or `operation` with summary → `evidenceRef` → named detail page.
The CLI `--technical-compilation-summary` flag is retired because the server summary is
the default. The server does not invent a causal lever or an AttributeUsage.

The current Build123d compilation profile is 3.1.0. It adds the narrow direct
scalar-leaf workspace-closure lowering boundary; `technical-compilation/2.0` remains the
...
| `build123d-closed-subset-v1` (`build123d-qualified-lezer` **1.7.0**)   | Python / Build123d     | Finite geometry algebra, numeric parameters, one solid `result`; standalone comments are provenance only | [CAD closed subset](../domains/cad/build123d-closed-subset-v1.md) |
