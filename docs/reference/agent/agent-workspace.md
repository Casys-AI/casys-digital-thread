...
results remain structured. `project_technical_compilation_preview` already returns the
bounded server-owned summary; use `project_technical_compilation_preview_detail` with
its opaque `evidenceRef` for a named page or explicit `full-evidence`. Full evidence is
...
| `project_capability_change_review`                     | Read / human amendment | Recompile the exact published-plan ceiling. A covered subset needs no prompt and does not shrink the ceiling. `withdrawUnused: true` may offer a strictly subtractive unused-authority withdrawal, confirmed with the same signed fingerprint retry. A widening, binding/digest/profile, or host-effect delta still requires that exact confirmation. No caller-selected runtime detail |
...
| `project_brief_requirements_review`          | None   | Complete traced `decisionParameters` for `model.write-requirements@2`; source-tested, native qualification pending                                                                                                                                                                                                                                                                                                               |
| `project_requirements_brief_trace_review`    | None   | Closed review for one late documentary correspondence. Accepts only `projectId`, `containerComponent`, `containerSourceItemId`, `requirementId`, and `sourceItemId`; returns exact `record.seal-requirements-brief-trace@1` parameters and current-basis evidence refs, or `unresolved`.                                                                                                                                         |
| `project_documentary_clause_response_review` | None   | Closed review for one source-backed documentary answer to one current approved brief item. Accepts `projectId`, `sourceItemId`, bounded `answer`/`scope`, and exact `sourceRefs` (full `agent-resource-capture/1.0` `resourceRef` or a current-basis Thread `artifactId`). Returns exact `record.seal-documentary-clause-response@1` parameters, or `unresolved`. Recording is not content acceptance, a requirement, or a pass. |
...
For requirements @2, that provenance is mandatory in the signed proposal and the
schema-5 capture, not just in this review response. Pass all returned parameters
verbatim. Proposal, queue and new dispatch reopen the exact approved brief and the
owning plan/change; a stale or missing source is refused. Completed replay reopens the
historical basis. Recapture @2 carries the exact provenance unchanged in schema 6;
legacy schemas 3/4 remain `TRACE GAP`. New @1 writing is retired, not a fallback.

The separate trace review can declare a late, versioned correspondence for one selected
native requirement from captures 3/4/5/6. It is provider-free and read-only: it derives
all scalar values and exact identities, then returns normal MRTR parameters for the
append-only `record.seal-requirements-brief-trace@1` document. The caller supplies no
threshold, source text, native target, provider or runtime. First claims bind one
current-basis `claimInput` artifact (the requirements capture); successors bind that
capture plus the previous claim. This is documentary, not satisfaction, semantic
equivalence, qualification, proof or an alteration of original provenance. Exact
historical replay revalidates the signed versions rather than consulting today's brief.
See
[requirements-to-brief correspondence qualification](../../how-to/extend/qualify-requirements-brief-trace.md).
The first real provider-free ID01 claim is separately documented; its original
`TRACE GAP` remains visible and no browser visual audit is implied by the BFF
declaration.
...
| `project_technical_compilation_preview`                   | Immutable evidence CAS | `{ projectId, sourceRefs: [capture.result.reference] }`. Returns a bounded canonical summary plus opaque `evidenceRef`; diagnostics, gaps, source material, projections, ready `decisionParameters` and the exact `compile.seal-admission@3` operation are read only through `project_technical_compilation_preview_detail`. `full-evidence` is explicit and required for MRTR. Reuse the detail operation verbatim in `project_change_append`. Build123d 3.0 lowers only the direct scalar-leaf closure; Modelica and circuit-only SPICE multi-file closures stay `unresolved` / `source.dependency-lowering-unavailable`. |
| `project_admitted_geometry_export`                        | Geometry **draft**     | Parameters for `design.write-geometry@1`. Not isolated execution                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                            |
| `project_admitted_geometry_export_preflight`              | None                   | Exact sealed admission identity only. Read-only singular-export readiness or safely derived independent child-root guidance; no source/profile/provider/runtime selector and no dispatch. Re-read/recross child heads after each later seal.                                                                                                                                                                                                                                                                                                                                                                                |
| `project_requirements_recapture_review`                   | None                   | `projectId`, plus optional exact `targetElementId` to select among captured PartDefinition families. Server derives current architecture/seed and active requirements predecessor; returns `model.recapture-requirements@1` for untraced schemas 3/4 or `@2` for traced schemas 5/6. Provider-free review, no native mutation, verdict or late claim.                                                                                                                                                                                                                                                                       |
| `project_requirements_brief_trace_review`                 | None                   | Five exact IDs only: `projectId`, `containerComponent`, `containerSourceItemId`, `requirementId`, `sourceItemId`. Reopens one canonical requirement and exact approved clause; returns normal MRTR parameters for `record.seal-requirements-brief-trace@1` and one/two exact current-basis input refs, or `unresolved`. No provider, native edit, threshold input, source text or proof.                                                                                                                                                                                                                                    |
| `project_documentary_clause_response_review`              | None                   | `projectId`, `sourceItemId`, bounded answer/scope, and exact sourceRefs only. Reopens the current human-approved brief item and every named agent-resource capture or Thread artifact; returns normal MRTR parameters for `record.seal-documentary-clause-response@1`, or `unresolved`. Caller URL+digest pairs are refused. Human MRTR authorizes recording the proposal, not accepting its content. No requirement, pass, provider or proof.                                                                                                                                                                              |
...
| `project_electrical_observation_method_sheet_seal_review` | None                   | `projectId` alone prepares the exact current L3 facts, Thread fingerprint and method-sheet identities; `projectId` + sheet fingerprint recrosses the captured sheet and returns the canonical MRTR for `verify.seal-electrical-observation-method-sheet@1`. No provider, threshold, verdict, ngspice call or L4                                                                                                                                                                                                                                                                                                             |
...
`project_technical_compilation_preview` takes exactly
`{ projectId, sourceRefs: [capture.result.reference] }`. For a multi-source request,
every opaque locator in `sourceRefs` must come from captures on one shared
ProjectSourceWorkspace basis; mixed workspace revisions are refused. Omitted `basis` is
the unique current Thread tip, not `latest`. Profile selection and SysML bindings are
unique server joins: one catalog profile per source role, `represents` only if there is
one `result` and one PartDefinition, `parameterizes` only if one AttributeUsage shares
the parameter name. Declare that AttributeUsage on `model.write-architecture@1` with
`attribute.<slug>.name` and `attribute.<slug>.parent`. Ambiguous or missing SysML stays
`binding.missing`. The preview result is a bounded summary of those join facts and an
opaque immutable `evidenceRef`. Call `project_technical_compilation_preview_detail` with
only `projectId`, that reference, one named section and an optional opaque cursor.
`full-evidence` is deliberately large only when explicit and remains mandatory for MRTR.
The server does not invent a causal lever or an AttributeUsage.

The current Build123d compilation profile is 3.1.0. It adds physical standalone module
comments as non-semantic provenance annotations to the narrow direct scalar-leaf
workspace-closure lowering boundary; `technical-compilation/2.0` remains the distinct
schema of the resulting compilation document. Capture and compile keep three facts
apart:
...
Only that Build123d 3.0 closure shape can lower a multi-file technical source. Modelica
and circuit-only SPICE multi-file closures remain `unresolved` /
...
| `model.write-requirements@1`                                                                            | historical only           | SysON                                                              | Historical integer scalar requirements; new planning and queueing are retired                                                                                       | A verdict or a provenance fallback                                                                           |
| `model.write-requirements@2`                                                                            | trusted                   | SysON                                                              | Traced integer scalar requirements and `requirements-capture/5.0`                                                                                                   | A verdict                                                                                                    |
| `compile.seal-admission@3`                                                                              | trusted                   | none                                                               | Admission capture                                                                                                                                                   | Execution authority                                                                                          |
| `model.recapture-requirements@1`                                                                        | trusted                   | SysON, read-only                                                   | Untraced schema-3/4 family recapture                                                                                                                                | Insert, delete, repair, solver call, evaluation or inherited verdict                                         |
| `model.recapture-requirements@2`                                                                        | trusted                   | SysON, read-only                                                   | Traced schema-5/6 family recapture, preserving exact original provenance                                                                                            | Insert, delete, repair, solver call, evaluation, late claim or inherited verdict                             |
| `record.seal-requirements-brief-trace@1`                                                                | trusted                   | none                                                               | One append-only documentary correspondence from one exact requirement to one exact approved brief clause; schemas 3/4/5/6                                           | Native mutation, satisfaction/equivalence, solver result or proof                                            |
| `record.seal-documentary-clause-response@1`                                                             | trusted                   | none                                                               | One append-only documentary proposal answering one exact current approved brief item from reopened immutable sources                                                | A requirement, content acceptance, verification pass, or `record.seal-requirements-brief-trace@1`            |
...
| `design.prepare-geometry-module@1`                                                                      | planning-only             | none (prerequisite-only)                                           | Internal preparation of the exact immediate-compound module; no caller binding                                                                                      | Geometry, evidence, verdict, provider, runtime, or execution grant                                           |
| `verify.observe-assembly-integrity@1`                                                                   | trusted                   | server-owned qualified runtime                                     | Factual `assembly-integrity-observation/1.0` over one exact current module; profile, method and runtime are server-owned                                            | A verdict, transform, tolerance choice, or provider selection                                                |
| `verify.evaluate-assembly-integrity@1`                                                                  | trusted                   | none                                                               | Code-owned L4 verdict capture over one exact L3 capture; never satisfies a gate                                                                                     | A caller-selected verdict, criteria, or gate satisfaction                                                    |
| `decide.accept-assembly-integrity-evaluation@1` / `decide.reject-assembly-integrity-evaluation@1`       | trusted, **human origin** | none                                                               | Human closeout of that exact L4; accept only when all five literal criteria are `pass`                                                                              | Safety, certification, a SysON/provider call, or a correction, CAD, or FEA grant                             |
| `verify.seal-prescribed-kinematics-case@1`                                                              | trusted                   | none                                                               | Sealed `prescribed-kinematics-case/1.0` from the exact workspace closure and architecture binding                                                                   | A provider/runtime/tool choice, CAD label, STEP inference, or dynamics claim                                 |
| `verify.run-prescribed-kinematics@1`                                                                    | trusted                   | authorized exact binding + exact host qualification + sealed ROP   | Factual `prescribed-kinematics-observation/1.0`; collision, clearance, contact, forces, torques, dynamics, strength, safety, manufacturability stay `not_evaluated` | A verdict, L5, or caller-selected provider/runtime                                                           |
| `verify.seal-prescribed-kinematics-method@1`                                                            | trusted                   | none                                                               | Sealed method sheet over the exact L1/L3 chain                                                                                                                      | A provider call, runtime choice, result value, or verdict                                                    |
| `verify.evaluate-prescribed-kinematics@1`                                                               | trusted                   | none                                                               | Deterministic L4 evaluation of the exact sealed case, L3 observation and method                                                                                     | A caller-selected fact, tolerance, or requested verdict                                                      |
| `decide.accept-prescribed-kinematics-evaluation@1` / `decide.reject-prescribed-kinematics-evaluation@1` | trusted, **human origin** | none                                                               | Human closeout of that exact L4; accept only on literal `pass`                                                                                                      | A dynamics, safety, manufacturing, or certification conclusion, or a correction, provider, CAD, or FEA grant |
...
Generic architecture authoring uses `model.write-architecture@1` or the agent-authored
`model.seal-architecture-sysml@1` closed-subset seal. Requirements writing @2, untraced
recapture @1, traced recapture @2 and provider-free documentary trace sealing are
...
| Requirement thresholds are integers                              | Canonical `requirement.<slug>.threshold` is a safe integer. SysON 0.5.1 cannot round-trip a decimal literal (`LiteralRational`). A traced brief `0.2 mm` is compiled to `200000 nm` with provenance `fractional-mm-to-nm`; do not pass `0.2 mm` as the canonical threshold. | Grammar rejects an uncanonicalised decimal at `project_decision_propose`. Message: threshold must be a safe integer because SysON 0.5.1 cannot round-trip decimal literals through `syson_constraint_extract`.                                                                                                                                                                                |
...
| `build123d-closed-subset-v1` (`build123d-qualified-lezer` **1.7.0**)   | Python / Build123d     | Finite geometry algebra, standalone provenance comments, numeric parameters, one solid `result` | [CAD closed subset](../domains/cad/build123d-closed-subset-v1.md) |
...
flowchart TD
  ...
  arch --> req["model.write-requirements@2 → requirements-capture/5.0"]
  req --> reqRecross["monotone architecture successor + exact active predecessor + new MRTR"]
  reqRecross --> reqCapture["model.recapture-requirements@2 → requirements-capture/6.0"]
  req --> trace["record.seal-requirements-brief-trace@1 → documentary claim only"]
  reqCapture --> trace
