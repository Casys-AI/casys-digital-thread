...
| `project_source_attachment_list`    | Read           | Active heads at one revision, filtered by exactly `fileId` or exactly target; includes `source-removed`                                                                                                                                                                |
...
command surface. After a source attachment, use `project_source_file_read`,
...
| `project_product_navigation_context`      | Read      | Selected node plus attachments grouped as sources, geometry, physics/cases, requirements/verdicts. Empty groups stay `unattached`.           |
| `project_product_source_closure`          | Read      | Technical dependency closure of one source **after** it is an exact attachment. Then use `project_source_file_read` / `project_source_tree`. |
