...
| Recorded DFM App `io.casys.mcp-dfm.results` | Sealed session over exact capture/case/STEP/run; `viewer.session.apply` only                            | Live mcp-dfm, a new numerical run, or restored r115/r117 MRTR authority                                |
| Historical `0.1` DFM capture                | Show `not recorded` / `unavailable` for later quality fields                                            | Inferred 0.3 quality numbers                                                                           |

## Buy

| This                                            | Is                                                                                                             | Is not                                                                              |
| ----------------------------------------------- | -------------------------------------------------------------------------------------------------------------- | ----------------------------------------------------------------------------------- |
| `buy.capture-configuration-cost@1`              | Signed ERP read of exact documents, CAS capture, documentary candidate bundle                                  | RFQ, PO, payment, BOM create, or approved spend                                     |
| `buy.seal-configuration-cost@1`                 | Provider-free Thread seal of the signed candidate bytes                                                        | A second ERP fetch or a current-price refresh                                       |
| `buy-configuration/1.0`                         | Sourced PartDefinition/occurrence identity with quantity/UOM                                                   | An arbitrary JSON attachment or the workspace catalogue                             |
| `buy-cost-bundle/1.0`                           | Dated selected cost lines with explicit coverage and gaps                                                      | A live ERP total or a Behave/Make verdict                                           |
| `erpnext_buy_capture`                           | Server-locked read-only provider tool                                                                          | `erpnext_bom_get`, caller-selected tool, or a fleet image tag                       |
| Catalogue `erpnext:item`                        | Workspace-declared identity                                                                                    | An attested Item mapping or a complete BOM                                          |
| Covered subtotal                                | Sum of priced lines with named included/excluded dimensions; `partial` may keep explicit unknown tax/transport | A fabricated zero line or a complete total when a required dimension is unknown     |
| Sealed Buy viewer session                       | Reopened sealed bytes + admitted whole-view package; `anchor` = `provenance.bundleRef`                         | Live ERP, `latest` package, `app.callServerTool`, or a hash of the displayed result |
| `project_buy_configuration_cost_capture_review` | Read-only preparation for capture MRTR                                                                         | Dispatch, consent, or `erpnext_buy_capture` chosen by the agent                     |
...
| `GET /api/projects` (`native-workbench-project-catalog/1.0`)            | All-or-nothing validated summaries; one invalid head → HTTP 503 `unavailable`                            | A partial list of healthy projects                                                       |
| `GET /api/project-discovery` (`native-workbench-project-discovery/2.0`) | Validated `store.get` available entries plus observed-storage unavailable placeholders                   | A silent skip, a `/projects/<id>` route, or a `projectId` query selector                 |
| `historicalEvaluations` / `sourceArtifacts[]`                           | Plural historical hops and observation provenance; a zero-evaluation first hop stays partial             | Current requirement `pass`, an automatic recross, or a new study                         |
