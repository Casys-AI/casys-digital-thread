...
| `mcp-view`              | Renderer-neutral MCP Apps lifecycle, small component registries/surfaces, shared primitives, and result-viewer scaffold. JSR has `0.4.1`; the sibling workspace contains the unpublished `0.5.0` candidate. | App source, built single-HTML resource, domain components, and the standalone default surface.                    | Server tools, a dashboard host, or evidence authority.                                     |
| `mcp-compose`           | Manifest/template-driven local MCP Apps host, iframe/capability boundary, explicit component-surface selection, and declared event routing. JSR has `0.7.1`; the local candidate is `0.8.0`.                | Dashboard layout, selected component instances, host-side declared tool grants, and event delivery.               | A source server's resources, component rendering, evidence, or hidden cross-server access. |
...
| Manufacturing BOM                 | ERPNext database behind `http://frontend:8080`; credentials are injected from an ignored env file                                                                                                   | The MCP exposes three agent categories with mutations; Compose grants only BOM list/detail reads.                  |
...
| ERPNext   | `http://127.0.0.1:3012/mcp` | Manufacturing BOM list and selected-document detail             |
