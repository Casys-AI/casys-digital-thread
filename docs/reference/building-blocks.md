...
| `mcp-view`               | Renderer-neutral MCP Apps lifecycle, optional-default component surfaces, Preact adapter, ERPNext-derived visual language, shared primitives, and scaffold. JSR has `0.5.0`; the sibling workspace contains the `0.6.0` candidate. | App source, built single-HTML resource, domain components, and optional standalone surface.                       | Server tools, a dashboard host, or evidence authority.                                     |
| `mcp-compose`            | Manifest/template-driven local MCP Apps host, iframe/capability boundary, explicit component-surface selection, and declared event routing. JSR has `0.8.0`; the local candidate is `0.8.1`.                                                   | Dashboard layout, selected component instances, host-side declared tool grants, and event delivery.               | A source server's resources, component rendering, evidence, or hidden cross-server access. |
...
| `mcp-build123d`          | Parametric CAD execution and export. The Compose service is `mcp-build123d` on port `3014`; its result App can inspect bounded GLB exports interactively.                                                                                      | Geometry programs, exact geometry metrics, CAD exports, and their local 3D presentation.                          | FEA solve results, dynamic system simulation, or product verdicts.                         |
...
| `mcp-erpnext`            | Privileged ERP engineering access. The Compose service is `mcp-erpnext` on port `3012` and currently builds a clean sibling checkout.                                                                                                          | Item/BOM creation, document lifecycle mutations, and real manufacturing, inventory, work-order and job-card data. | SysML part authority, CAD geometry, computed physics, or a requirement verdict.            |
| `mcp-erpnext-components` | Product-specific read-only presentation service on port `3017`, implemented in this workspace with Preact and `mcp-view`.                                                                                                                      | One BOM surface result and six atomic dashboard components.                                                       | ERP mutation, provider-native agent API, or standalone dashboard policy.                   |
...
| `casys-digital-thread`   | This control-plane workspace: read-only Console, fleet configuration, Compose declarations, scenario plan, and docs.                                                                                                                           | Console source and bundle, manifests, dashboard component surfaces, observation code, labelled demo fixtures.     | Direct solver execution, persistent Modelica data, CAD exports, or SysON model mutation.   |

With Deno 2.9, a JSR package published less than 24 hours ago may be held by the default
minimum dependency age. If `@casys/mcp-view@0.5.0` is temporarily affected, keep the
...
| Manufacturing BOM                 | ERPNext database behind `http://frontend:8080`; credentials are injected from an ignored env file                                                                                                   | Broad MCP owns mutations; the product palette grants only `erpnext_bom_surface`.                                   |
...
All five engineering MCP endpoints are loopback-only on the host:
...
| ERPNext agent API  | `http://127.0.0.1:3012/mcp` | Provider-native ERP tools and mutations                         |
| ERPNext components | `http://127.0.0.1:3017/mcp` | Read-only BOM composition data                                  |
