...
| `mcp-view`               | Renderer-neutral MCP Apps lifecycle, optional-default component surfaces, shared Preact primitives and scaffold. Domain Apps consume the versioned presentation kit instead of copying a local design system. | App source, built single-HTML resource, domain components, and optional standalone surface.                       | Server tools, a dashboard host, or evidence authority.                                     |
| `mcp-compose`            | Manifest/template-driven local MCP Apps host, iframe/capability boundary, explicit component-surface selection, and declared event routing. The workspace consumes npm `0.8.1`.                               | Dashboard layout, selected component instances, host-side declared tool grants, and event delivery.               | A source server's resources, component rendering, evidence, or hidden cross-server access. |
...
minimum dependency age. If `@casys/mcp-view@0.7.0` is temporarily affected, keep the
