...
| `mcp-view`              | Browser-side MCP Apps primitives and the standard result-viewer scaffold. `0.4.0` is the target version for the scaffold in this workspace. | App source, built single-HTML resource, structured-result rendering.                                            | Server tools, a dashboard host, and evidence authority.                                  |
...
`0.4.0` above is a **target/local contract**, not a claim that a package resolver can
already fetch it. Resolve the version in the environment that will build it; do not
change the repository's global dependency-age policy merely to make one scaffold command
work.
