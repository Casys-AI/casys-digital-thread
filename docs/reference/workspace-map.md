...
This page is the lookup reference for the control-plane workspace. It names
the source of each fact so that desired configuration, observed runtime state,
demo evidence, and a scenario contract cannot be confused.
...
All bindings below are loopback-only on the host. Container services listen on
their internal ports only so that Compose can route between them.
...
The console defaults to `MCP_HOSTNAME=127.0.0.1` and `MCP_PORT=3020`; the
server also accepts `--hostname` and `--port`. The preview harness defaults to
`3021` and targets the console on `3020`.
...
`console_run_detail`, and `console_refresh`. It is not a general MCP proxy.
It supplies the MCP Apps host capability needed by the fixed view to make those
read-only server calls; another host must supply an equivalent capability to
render the view live.

## CoffeeMachine scenario-contract binding

The plan `coffee-machine-nominal-v1` is provisional and contains exactly one
condition:
...
The evaluator attaches it only when all of the following match the persisted
Modelica evidence:
...
The live call is `syson_constraint_evaluate`. A mismatch causes no comparison
to be attached (`not_evaluated`); malformed or unavailable evaluation becomes
an `error`, never an optimistic pass.

This is not a product requirement and is not a requirement stored in a SysON
project. The threshold comes from the scenario itself, while the horizon only
identifies the scenario. Product requirements must be modelled and traced
separately in SysON.
...
- **`not_evaluated`**: execution evidence exists but no eligible comparison is
  attached.
- **`passed` / `failed` / `unresolved` / `error`**: comparison state. It never
  changes the fact of whether the simulation executed.

## Composition boundary

The current console is one fixed MCP App built with `@casys/mcp-view`.
`@casys/mcp-compose` is the planned separate composition layer for generated
layouts, multi-server resource bridging, and cross-panel event/session routing.
The local browser preview does not implement those capabilities.
