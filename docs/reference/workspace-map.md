...
| [`config/compose/dashboards/coffee-machine-cm01.yaml`](../../config/compose/dashboards/coffee-machine-cm01.yaml)             | Saved SysON, build123d, ERPNext and Modelica calls, stable component IDs, presentation intent and 2×2 fallback layout | Replaying the first product dashboard without storing its environment-specific IDs   |
...
| Manufacturing BOM                    | ERPNext database on the external ERPNext Docker network       | `erpnext_bom_list` / `erpnext_bom_get`; credentials remain runtime-only              |
...
backed by the sibling `@casys/mcp-compose@0.8.0` candidate until its component-surface
...
from zero. Its separate Compose manifest grants the Doclist viewer only
`erpnext_bom_list` and `erpnext_bom_get`; dashboard code cannot call the mutation tools.
