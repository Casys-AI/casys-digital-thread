...
profile 3.1.0 adds exact direct scalar-leaf workspace-closure lowering and accepts
physical standalone-module comments only as non-semantic provenance annotations. It
requires a parser-reported finite module-level numeric parameter bound through
`parameterizes` and causally reaching the unique `result`; it has no legacy-profile
