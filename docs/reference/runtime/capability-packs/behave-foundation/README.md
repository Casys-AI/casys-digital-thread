...
| `contract-attested` | In addition to exact local material, the mandatory MCP endpoints declared `/health`, `server/discover`, `tools/list` and `resources/list`; the listed input/output schemas are fingerprinted. | A tool execution, provider side effect, canonical geometry or static-FEA proof. |
...
only. A schema fingerprint is an `observed-not-verified` identity of that live
