...
`syson-app` and the dedicated `mcp-syson@0.8.3` image add no named volume in this
closure. The `chain` network is runtime topology, not stored evidence. The CalculiX
microVM is ephemeral; its durable attempt, CAS, WAL and Thread evidence stay under the
Digital Thread server-owned stores and are outside pack removal.
