...
concrete binding and atomic unit set. It is an inspection map, not an agent selector: the
server selects a binding through policy and the trusted catalogue. Digests, host mode,
availability, and host effects stay in the exact proposal and atomic catalogue rather
...
[`first-party-capability-binding-catalog.ts`](../../../../src/adapters/control-plane/first-party-capability-binding-catalog.ts), not this table.
