...
| `http://127.0.0.1:3015/mcp`  | `mcp-calculix`              | Pinned HTTP `0.8.1` sensitivity fleet: mesh preflight plus static, modal, buckling, creep and coupled-thermal FEA; identity-bound recorded static runs. `deno task probe:calculix-contract` reads only health, discovery and schemas — never `tools/call`. It is not product static `verify.run-fea-static-proof@3`. |
