...
The first-party H1 launch-group registry enrolls six persistent topologies:
`casys-syson@1.0.1`, `casys-build123d-sandbox@1.0.0`,
`casys-build123d-observation@1.0.0`, `casys-chrono@1.0.0`,
`casys-mcp-calculix@1.0.0`, and `casys-mcp-dfm@1.0.0`. Enrollment is candidacy. It does not start a service at
...
The immutable repository catalogue baseline for `casys.mcp-chrono@0.3.5` remains
`unqualified` (Linux/amd64 only). This repository does not claim a matching host
attestation for digest
`sha256:a56fc994c6ff6beb33b884a7a638cd5fc00281201f9ad5e5556ba790adb39c18`; inspect or
...
qualify this 0.3.5 material or rewrite the catalogue. HTTP `casys.mcp-calculix@0.8.5`
...
| H1 JIT launch group (`casys-syson`, `casys-build123d-sandbox`, `casys-build123d-observation`, `casys-chrono`, `casys-mcp-calculix`, `casys-mcp-dfm`) | `CapabilityRuntimeSupervisor` | Product path after operational authorization. Separate Compose project names. Registry enrollment is candidacy; a group starts only under a demanded lease. |
...
| `http://127.0.0.1:3014/mcp` | `mcp-build123d`                  | Dedicated HTTP `0.7.0` provider for server-fixed CAD recipes and shared exports. `deno task probe:build123d-contract` reads health, discovery, schemas and viewer declarations only; it never calls a provider tool and is not canonical geometry.                                                                                                                                                                                                                            |
| `http://127.0.0.1:3024/mcp` | `mcp-build123d-sandbox`          | Same dedicated `0.7.0` provider identity, but agent-proposed geometry preview stays on its private export volume. It is not canonical geometry.                                                                                                                                                                                                                                                                                                                               |
| `http://127.0.0.1:3025/mcp` | `mcp-chrono`                     | Fixed server-owned prescribed-kinematics binding only. The agent never calls this endpoint or supplies its bearer/tool/arguments. Catalogue baseline is `unqualified`; no host qualification is claimed for digest `sha256:a56fc994c6ff6beb33b884a7a638cd5fc00281201f9ad5e5556ba790adb39c18`.                                                                                                                                                                                 |
| `http://127.0.0.1:3015/mcp` | `mcp-calculix`                   | Sealed `casys-mcp-calculix@1.0.0` sensitivity group in Compose project `casys-mcp-calculix-v1`, not root Compose. It has no Docker healthcheck and its sole sealed readiness is MCP `tools/list` (15 s total, 1 s per attempt, 250 ms retry); it exposes no `/health`. Its binding is currently unqualified/non-activable, so fleet projection remains literally unavailable. A later qualification may activate its fixed recorded-run protocol. Never product static proof. |
...
| `http://127.0.0.1:3018/mcp` | `mcp-dfm`                        | Sealed `casys-mcp-dfm@1.0.0` measured-checks group in Compose project `casys-mcp-dfm-v1`, not root Compose. Binding `mcp-dfm-measured-checks` is catalogue-`qualified` for `manufacturing.run-dfm-checks@1`. Tools `dfm_check_envelope`, `dfm_check_min_thickness`, `dfm_check_overhangs`; SHA-256 attestation required. Live tools take `step_path`, not STL. Root Compose on the same port collides.                                                                                                                                       |
...
| `http://127.0.0.1:3023/mcp` | `mcp-spice`                      | Digest-pinned 0.6.2 ngspice netlist admission, OP, reduced transients, bounded DC sweeps, and durable readbacks; maintainer D2 probe only, never the admitted product path                                                                                                                                                                                                                                                                                                                       |
...
| Recorded FEA staging         | CalculiX `calculix-inputs:/inputs` volume   | After an active exact capability lease, the server stages content-addressed STEP bytes through the owned digest-pinned container; provider-private, non-authoritative, not evidence                                                                                 |
| CalculiX recorded runs       | `calculix-runs:/var/lib/mcp-calculix-runs`  | Exact request-id `calculix_run_get` plus nine `resources/read` captures; separate from CAD exchange and retained by the launch group                                                                                                                                |
| CalculiX image export mount  | `calculix-exports:/exports` volume          | Required by the CalculiX image, private and retained by the launch group. It is never proof/evidence and never a CAD exchange volume.                                                                                                                               |
