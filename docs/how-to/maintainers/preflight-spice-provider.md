...
Treat `non-executable-preflight` with `unresolved` literally. For the qualified 0.6.2
surface, the read checks the seven-tool schema set (netlist submission, OP, transient,
DC sweep, and three durable readbacks), declared 1 MiB netlist ceiling on both paths, 32-per-kind observables,
...
execution, a replayable electrical run, a typed `tools/call` error, L4, or L5. W
remains unresolved and the listed readbacks stay uncalled. After an unknown external
effect, quarantine for human review; never redispatch from this probe.

The emitted `desired.release` records the exact configured 0.6.2 OCI index, revision,
