...
aucune source de ces serveurs. Ce repo contient en revanche la source du cockpit MCP
read-only (`server.ts`, `src/`). Pour éditer un serveur d'ingénierie, cloner son repo
...
Composition type : `build123d_export` écrit `/exports/piece.step` →
`calculix_solve_static` le lit au même chemin → le résultat (masse, contrainte) se
vérifie contre le modèle via `syson_constraint_evaluate` avec valeurs unitées
(`{"value": 0.0569, "unit": "kg"}`). Les runs Modelica restent dans leur volume dédié
`/runs` : ils ne partagent ni export CAD ni socket Docker.
...
`config/compose/` déclare déjà son premier panneau `@casys/mcp-compose` : le host local
lit la ressource par MCP `resources/read` et n'accorde que les tools read-only marqués
`appCallable`. Ce n'est pas encore un Workbench multi-panneau ni du routage d'événements
: toute extension doit rester déclarée dans le manifest et le template.
...
- **Desired ≠ observed** : le manifeste ne prouve jamais qu'un service tourne. Les
  ...
  `unavailable`.
- **Demo ≠ live** : le run bracket est une preuve de démonstration hashée. Son FEA
  `26.6 MPa` est un résultat documenté, jamais présenté comme un solve fraîchement
  exécuté.

## État et prochaine étape

Chaîne complète publiée le 2026-07-30 (voir `docs/positioning.md` pour le SOTA). **Le
chantier décisif est `experiments/oracle/`** : mesurer le taux de correction de l'agent
modeleur avec et sans oracle — le benchmark Hephaestus-CCX (50 cas, kits CalculiX, arXiv
2605.17448) fournit un protocole tout prêt. C'est ce qui fait passer le projet du niveau
« ancré, vérifié » au niveau « boucle fermée » de l'échelle de maturité neurosymbolique.
