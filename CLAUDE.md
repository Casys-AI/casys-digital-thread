...
Ce repo est **l'atelier** de la chaîne « executable digital thread » : exigence → modèle SysML v2 → géométrie → physique → preuve. Les serveurs MCP d'ingénierie vivent dans leurs propres repos et s'exécutent depuis l'image `ghcr.io/casys-ai/engineering-toolchain` — on ne clone ici aucune source de ces serveurs. Ce repo contient en revanche la source du cockpit MCP read-only (`server.ts`, `src/`). Pour éditer un serveur d'ingénierie, cloner SON repo (`Casys-AI/mcp-syson`, `mcp-build123d`, `mcp-calculix`, `constraint-solver`).
