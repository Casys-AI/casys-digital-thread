...
- `modelica_kit_list` / `modelica_simulate` / `modelica_run_list` / `modelica_run_get` — simulation système OpenModelica de kits approuvés : température, temps, énergie et puissance. Le cockpit découvre les records via les deux outils de lecture, jamais via le volume Docker. Une simulation réussie produit des observations et artefacts hashés ; le verdict reste du ressort de SysON + `constraint-solver`.
