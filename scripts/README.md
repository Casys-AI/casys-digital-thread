...
`--run` (and `--recover` where the worker owns a durable WAL). Build123d, geometry,
Modelica and ngspice also accept `--retry-infrastructure-failure` for exactly one
successor after a proven not-published, destroyed predecessor; CalculiX does not. They
reuse the production composition and never build or delete images. The Modelica
candidate gate always owns both server-owned proofs and does not replace the Docker
preflight or the active-pin vertical. The ngspice candidate gate always owns the
admitted-circuit resistor-divider proof and does not replace the Docker smoke.
