...
end. Imported first-party CAD candidates use the `*-candidate-qualification.ts` gates
with only `--import-record` plus `--run`; they reuse the production composition and
never build or delete images.
...
and
[Qualify a first-party CAD microVM image candidate](../docs/how-to/maintainers/qualify-a-first-party-cad-microvm-image-candidate.md).
